...
#define REG_SET_VALUE(__base,__v) 		((*(volatile u32 *)(__base)) = (uint32_t)(__v))
...
#define CTL_BASE_AHB    	0xE2210000
#define CTL_BASE_SCE    	0xC0D00000//secure
#define CTL_BASE_PCE    	0xC0E00000//public

//ce clk register
#define SPRD_APCLK_BASE				0xE2000000
#define REG_AP_CLK_CGM_CE0_CFG		(SPRD_APCLK_BASE + 0x0038)
...
//#define CTL_BASE_SCE    	0x50200000//secure
//#define CTL_BASE_SCE    	0x50300000//public
...
static void sprd_flush_dcache_range(unsigned long start, unsigned long stop) {
  uint32_t start_addr = start & (~63);
  uint32_t end_addr   = (stop) & (~63);
  uint32_t clflush_addr;
 
  __asm volatile("mfence");
  for(clflush_addr = start_addr; clflush_addr <= end_addr; clflush_addr += 64)
  {
    __asm volatile("clflush %0" : "+m"(*(volatile char *)clflush_addr));
  }
  __asm volatile("mfence");
}
...
int sprd_ce_set_data_phy(uint32_t mode, struct sprd_ce_data *p_data) {
    ...
    g_dma.src_addr = 0x00000000ffffffff & (uint32_t)p_data->in;
    ...
    if (g_dma.src_cacheable == true) {
        //flush_dcache_range((unsigned long)p_data->in, (unsigned long)(p_data->in + p_data->in_len - 1));
        //wbinvd();
        sprd_flush_dcache_range((unsigned long)p_data->in, (unsigned long)(p_data->in + p_data->in_len - 1));
    }

    g_dma.dst_addr = 0x00000000ffffffff & (uint32_t)p_data->out;
    ...
    if(g_dma.dst_cacheable == true) {
        //invalidate_dcache_range((unsigned long)p_data->out, (unsigned long)(p_data->out + p_data->out_len - 1));
        //flush_dcache_range((unsigned long)p_data->out, (unsigned long)(p_data->out + p_data->out_len - 1));
        //wbinvd();
        sprd_flush_dcache_range((unsigned long)p_data->out, (unsigned long)(p_data->out + p_data->out_len - 1));
    }
...
int sprd_ce_set_key_phy(uint32_t mode, struct sprd_ce_key *p_key) {
    ...
    g_dma.key_addr = 0x00000000ffffffff & (uint32_t)p_key->key;
    ...
    if ((flush == true) && (g_dma.key_cacheable == true)){
        //flush_dcache_range((unsigned long)p_key->key, (unsigned long)(p_key->key + p_key->key_len - 1));     
        //wbinvd();
        sprd_flush_dcache_range((unsigned long)p_key->key, (unsigned long)(p_key->key + p_key->key_len - 1));
    }
...
int sprd_ce_polling(void) {
    while (1) {
        if (REG_GET_VALUE(SPRD_CE_INT_STATUS) & (SPRD_STD_DONE_INT|SPRD_STD_END_INT)) {
...
void sprd_ce_reset(void) {
    ...
    REG_SET_VALUE(REG_AP_AHB_RF_AHB_RST,
            REG_GET_VALUE(REG_AP_AHB_RF_AHB_RST)
            | BIT_AP_AHB_RF_CE0_SOFT_RST);
