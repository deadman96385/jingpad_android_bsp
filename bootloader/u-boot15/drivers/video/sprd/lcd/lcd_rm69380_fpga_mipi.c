...
static int rm69380_init(void)
{
	...
	printk("MD6000: MD6000_LOGIC_RST HIGHT 1\n");
	sprd_gpio_direction_output(NULL, MD6000_LOGIC_RST, 0);
	printk("MD6000: MD6000_LOGIC_RST LOW\n");
	sprd_gpio_direction_output(NULL, MD6000_LOGIC_RST, 1);
	printk("MD6000: MD6000_LOGIC_RST HIGHT 2\n");
...
{ 
    sprd_gpio_request(NULL, CONFIG_FPGA_GPIO_1V2_LDO);
    sprd_gpio_direction_output(NULL, CONFIG_FPGA_GPIO_1V2_LDO, 0);
    mdelay(5);
    regulator_disable("vddldo2");
    mdelay(5);
    sprd_gpio_request(NULL, CONFIG_FPGA_GPIO_RST_LDO);
    sprd_gpio_direction_output(NULL, CONFIG_FPGA_GPIO_RST_LDO, 0);
    mdelay(5);
    sprd_gpio_request(NULL, CONFIG_FPGA_GPIO_1V8_LDO);
    sprd_gpio_direction_output(NULL, CONFIG_FPGA_GPIO_1V8_LDO, 0);
    mdelay(5);
...
#define CONFIG_FPGA_GPIO_RST_LDO  137 
    ...
    if (on) {
        sprd_gpio_request(NULL, CONFIG_FPGA_GPIO_1V2_LDO);
        sprd_gpio_direction_output(NULL, CONFIG_FPGA_GPIO_1V2_LDO, 1);
        mdelay(5);
        regulator_set_voltage("vddldo2",3300);
        regulator_enable("vddldo2");
        mdelay(5);
        sprd_gpio_request(NULL, CONFIG_FPGA_GPIO_RST_LDO);
        sprd_gpio_direction_output(NULL, CONFIG_FPGA_GPIO_RST_LDO, 1);
        mdelay(5);
        sprd_gpio_request(NULL, CONFIG_FPGA_GPIO_1V8_LDO);
        sprd_gpio_direction_output(NULL, CONFIG_FPGA_GPIO_1V8_LDO, 1);
        mdelay(5);
...
static struct panel_info rm69380_info = {
	/* common parameters */
	.lcd_name = "lcd_rm69380_fpga_mipi_2k",
