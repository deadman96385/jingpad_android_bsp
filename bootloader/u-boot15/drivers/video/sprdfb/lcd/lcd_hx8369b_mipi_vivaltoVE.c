...
{
	int32_t i;
	LCM_Init_Code *init = init_data_boe_hf;
	unsigned int tag;
	...
	LCD_PRINT("lcd_hx8369b_init vivaltoVE\n");

	mipi_set_cmd_mode();
	mipi_eotp_set(0,0);

	for(i = 0; i < ARRAY_SIZE(init_data_boe_hf); i++){
		tag = (init->tag >>24);
		if(tag & LCM_TAG_SEND){
...
static uint32_t hx8369b_readid(struct panel_spec *self)
{
	...
	for(j = 0; j < 4; j++){
		...
		if((0x55 == read_data[0])&&(0xBF == read_data[1])&&(0x90 == read_data[2]))
		{
			return 0x8369;
		}
	}
...
struct panel_spec lcd_hx8369b_mipi_vivaltoVE_spec = {
	.width = 480,
	.height = 800,
	.fps = 60,
	.type = LCD_MODE_DSI,
