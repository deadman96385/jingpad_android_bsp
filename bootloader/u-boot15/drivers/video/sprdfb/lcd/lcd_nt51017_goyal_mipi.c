...
static int32_t nt51017_mipi_init(struct panel_spec *self)
{
	...
	sprd_gpio_request(NULL, 152);
	sprd_gpio_direction_output(NULL, 152, 0);
...
static uint32_t nt51017_readid(struct panel_spec *self)
{
	...
	LCD_PRINT("lcd_nt51017_mipi read id!\n");
	...
	for(j = 0; j < 4; j++){
		...
		LCD_PRINT("lcd_nt51017_mipi read id 0xda value is 0x%x!\n",read_data[0]);

		read_rtn = mipi_force_read(0xdb,1,&read_data[1]);
		LCD_PRINT("lcd_nt51017_mipi read id 0xdb value is 0x%x!\n",read_data[1]);

		read_rtn = mipi_force_read(0xdc,1,&read_data[2]);
		LCD_PRINT("lcd_nt51017_mipi read id 0xdc value is 0x%x!\n",read_data[2]);

		if((0x55 == read_data[0])&&(0xbf == read_data[1] || 0xc0 == read_data[1] || 0xbe == read_data[1])&&(0x90 == read_data[2])){
				LCD_PRINT("lcd_nt51017_mipi read id success!\n");
				return 0x8369;
		}
	}
