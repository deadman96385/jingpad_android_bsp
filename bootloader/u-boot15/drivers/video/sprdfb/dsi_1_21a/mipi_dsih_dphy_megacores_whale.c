...
#if defined(CONFIG_SOC_SHARKLE) || defined(CONFIG_SOC_SHARKLJ1) || defined(CONFIG_SOC_PIKE2)
	union __reg_0f__ {
		struct __0f {
			u8 det_delay:2;
			u8 kdelta: 4;//12 - 15 -> bit2 - bit5
			u8 ldo0p4:2;
		} bits;
		u8 val;
	} _0f;
...
extern uint32_t mipi_dsih_current_versions;
...
static int mipi_dsih_dphy_calc_pll_param(dphy_t *phy, struct dphy_pll *pll)
{
	...
	for (i = 0; i < 4; ++i) {
		if (pll->potential_fvco >= VCO_BAND_LOW && pll->potential_fvco <= VCO_BAND_HIGH) {
	...
	if (pll->fvco >= VCO_BAND_LOW && pll->fvco <= VCO_BAND_MID) {
		...
		if(0x100 == mipi_dsih_current_versions)
			pll->lpf_sel = 1;
		else
			pll->lpf_sel = 7;
...
#else
	if(0x100 == mipi_dsih_current_versions)
		pll->refin = 0; /* pre-divider bypass */
	else
		pll->refin = 3;
#endif
...
#if defined(CONFIG_SC9833)
		delta_freq = delta_freq / khz * (i + 1);
		pll->kstep = pll->ref_clk * hopping_period;
		pll->kdelta = (((1 << 20) * delta_freq / pll->ref_clk / pll->kstep) -3) / 4;
#elif defined(CONFIG_SOC_SHARKLE) || defined(CONFIG_SOC_SHARKLJ1) || defined(CONFIG_SOC_PIKE2)
		delta_freq = delta_freq / khz * (i + 1);
		pll->kstep = pll->ref_clk * hopping_period;
		pll->kdelta = ((1 << 20) * delta_freq / pll->ref_clk / pll->kstep);
#else
		pll->kstep = pll->ref_clk * hopping_period;
...
#if defined(CONFIG_SC9833)
		delta_freq = pll->freq / khz * 0.015 *2 * (i + 1);
		pll->kstep = pll->ref_clk * hopping_period;
		pll->kdelta = (((1 << 20) * delta_freq / pll->ref_clk / pll->kstep) -3) / 4;
#elif defined(CONFIG_SOC_SHARKLE) || defined(CONFIG_SOC_SHARKLJ1) || defined(CONFIG_SOC_PIKE2)
		delta_freq = delta_freq / khz * (i + 1);
...
{
	/* currently, dphy on pike only support 2 lanes */
#if 0
