...
static const uint32_t mipi_dsih_supported_versions[] = {0x100, 0x200, 0x3132302A, 0x3132312A, 0x3133312A};
static const uint32_t mipi_dsih_no_of_versions = sizeof(mipi_dsih_supported_versions) / sizeof(uint32_t);
uint32_t mipi_dsih_current_versions;
...
dsih_error_t mipi_dsih_open(dsih_ctrl_t * instance)
{
    ...
    else
    {
        ...
        for (i = 0; i < mipi_dsih_no_of_versions; i++)
        {
            ...
            {
		mipi_dsih_current_versions = version;
                break;
            }
        }
    ...
    printf("phy status %x\n",instance->phy_instance.core_read_function(DSI_CTL_BEGIN, R_DSI_HOST_PHY_STATUS));
    ...
    mipi_dsih_hal_int_mask_0(instance, 0x0);
    mipi_dsih_hal_int_mask_1(instance, 0x0);
#else
    mipi_dsih_hal_int_mask_0(instance, 0xffffffff);
    mipi_dsih_hal_int_mask_1(instance, 0xffffffff);
#endif
...
dsih_error_t mipi_dsih_dpi_video(dsih_ctrl_t * instance, dsih_dpi_video_t * video_params)
{
    ...
    mipi_dsih_hal_dpi_sig_delay(instance, 95*(video_params->h_total_pixels * ratio_clock_xPF) / PRECISION_FACTOR /100);
#endif
...
#if defined(CONFIG_SC9833) || defined(CONFIG_SOC_SHARKLJ1) || defined(CONFIG_SOC_SHARKLE) || defined(CONFIG_SOC_PIKE2)
    mipi_dsih_dphy_enable_hs_clk(&(instance->phy_instance), 1);//jessica, panel not init yet, could not set to hs, or result in err to some panel
    mipi_dsih_reset_controller(instance);
#endif

    mipi_dsih_non_continuous_clock(instance, instance->non_continuous_clock);
...
uint16_t mipi_dsih_gen_rd_packet(dsih_ctrl_t * instance, uint8_t vc, uint8_t data_type, uint8_t msb_byte, uint8_t lsb_byte, uint8_t bytes_to_read, uint8_t* read_buffer)
{
    ...
    for (timeout = 0; timeout < DSIH_FIFO_ACTIVE_WAIT; timeout++)
    {
        ...
        if (!mipi_dsih_hal_gen_rd_cmd_busy(instance))
        {
            ...
            else if(timeout == (DSIH_FIFO_ACTIVE_WAIT - 1))
            {
                if (instance->log_error != 0)
                {
