...
typedef union usb_grxstsp_tag
{
    struct usb_grxstsp_map
    {
        ...
        volatile uint32_t  frame_number                    :4;  //frame nummber
        volatile uint32_t  packet_status                   :4;  //packet status
        volatile uint32_t  data_pid                        :2;  //data PID
        volatile uint32_t  byte_count                      :11; //byte count
        volatile uint32_t  endpoint_number                 :4;  //endpoint nummber
      #else
	       volatile uint32_t 	endpoint_number 				:4;  //endpoint nummber
	       volatile uint32_t 	byte_count						:11; //byte count
	       volatile uint32_t 	data_pid						:2;  //data PID
