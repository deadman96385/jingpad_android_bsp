...
static void *sprd_get_base(struct i2c_adapter *adap)
{
	return sprd_i2c[adap->hwadapnr].base;
}

static void sprd_i2c_apb_enable(struct i2c_adapter *adap)
{
	unsigned int tmp = readl(sprd_i2c[adap->hwadapnr].apb_base);

	writel(tmp | (sprd_i2c[adap->hwadapnr].apb_eb),
			sprd_i2c[adap->hwadapnr].apb_base);
}
...
static int sprd_wait_for_fifo_empty(void *i2c_base)
{
	...
	int status = readl(i2c_base+I2C_STATUS);
...
static int sprd_i2c_write_byte(void *i2c_base, uchar *byte, int c)
{
	int i;
...
static int sprd_i2c_read_byte(void *i2c_base, uchar *byte, int c)
{
	int i;
			    ...
			    int alen, uchar *buffer, int len)
{
	...
	res = sprd_i2c_write_byte(i2c_base, (uchar *)&addr, 1);
