...
/* HW Define */
#define WCN_26M_FROM_TCXO 0
#define WCN_26M_FROM_REFOUT 1
...
void dcdc_ldo_power_on()
{
	...
	//regulator_disable("vddrf1v8");  //used by ORCA refered by EXT_XTL_EN4 and ROC1 NTC not use
	...
	//regulator_enable("vddldo1"); // for WCN ANT, only on by WC_CLK_REQ
...
static void pmic_lowpower_config(void)
{
	...
	ANA_REG_SET(ANA_REG_GLB_SLP_LDO_PD_CTRL0,
		BIT_SLP_LDO_VDDRF1V8_PD_EN |  //ORCA refered by EXT_XTL_EN4 and ROC NTC not use
	...
	ANA_REG_SET(ANA_REG_GLB_SLP_LDO_PD_CTRL1,
		...
		BIT_SLP_LDO_VDDLDO1_PD_EN |  //marlin ANT use
		0
	);
...
// default config wcn use TCXO,
// reconfig refout if wcn clk from refout4
static void sprd_refout_reconfig(void)
{
	/* REFOUT4 binding WB_CLK_REQ */
	ANA_REG_OR(ANA_REG_GLB_TSX_CTRL2, BIT_DCXO_26M_REF_OUT4_EXT_XTL3_EN);
	/* REFOUT4 binding NR sleep status */
	ANA_REG_OR(ANA_REG_GLB_TSX_CTRL3, BIT_DCXO_26M_REF_OUT4_EXT_XTL4_EN);
	/* REFOUT2 binding NR sleep status */
	ANA_REG_OR(ANA_REG_GLB_TSX_CTRL3, BIT_DCXO_26M_REF_OUT2_EXT_XTL4_EN);
}

void sprd_pmu_lowpower_init(void)
{
	unsigned int wcn_clk_sel = -1;
	...
	wcn_clk_sel = sprd_get_wcn_sel();
	if (wcn_clk_sel == WCN_26M_FROM_REFOUT)
		sprd_refout_reconfig();
