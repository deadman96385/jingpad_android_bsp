...
#define REG_ADDR_BOARD_ID1_GPIO REG_MISC_PIN_SD2_CLK + SPRD_PIN_PHYS
#define BOARD_ID1_GPIO  134
#define BOARD_ID2_GPIO  137
#define BOARD_ID3_GPIO  135
...
/* enable 20k pull up */
static void gpio_config_enable_pull(unsigned int reg_addr)
{
...
/* disable 20k pull up */
static void gpio_config_disable_pull(unsigned int reg_addr)
{
	/*disable gpio160 pullup&pulldown*/
	__raw_writeb(0 << 6, reg_addr);
...
unsigned int sprd_get_hw_version(void)
{
	unsigned int gpio_val1 = -1;
	unsigned int gpio_val2 = -1;

	gpio_config_enable_pull(REG_ADDR_BOARD_ID1_GPIO);
	gpio_val1 = gpio_state(BOARD_ID1_GPIO);
	if(gpio_val1 == 1) {
		gpio_config_disable_pull(REG_ADDR_BOARD_ID1_GPIO);
		return 1;
	} else if (gpio_val1 == 0) {
		gpio_config_disable_pull(REG_ADDR_BOARD_ID1_GPIO);
...
unsigned int sprd_get_wcn_sel(void)
{
	unsigned int gpio_val1 = -1;

	gpio_val1 = gpio_state(BOARD_ID3_GPIO);
	if (gpio_val1) {
		return 1;
	} else {
		return 0;
