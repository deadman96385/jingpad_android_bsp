#include <secureboot/sec_efuse_orca_drv.h>
#include <asm/arch/sys_timer_reg_v0.h>
#include <asm/arch/chip_orca/aon_apb.h>
...
Efuse_Result_Ret efuse_write_drv(uint32 start_id, uint32 end_id, uint32 *pReadData,uint32 Isdouble)  //write and set lockbit
{
	uint32  i, j;
...
Efuse_Result_Ret efuse_rewrite_double_bit(uint32 block_id, uint32 writeData) //write but not set lockbit
{
	Efuse_Result_Ret ret = EFUSE_RESULT_SUCCESS;
	efuse_lock();
	efuse_enable();
