...
int microcode_update(void)
{
	...
	do {
		...
		address = (uint32_t) update.data + UCODE_HEADER_LEN;
		...
		if (update.update_revision != rev) {
			...
			gd->arch.microcode_data = update.data;
			gd->arch.microcode_size = update.size;
		}
		count++;
	} while (1);
}
