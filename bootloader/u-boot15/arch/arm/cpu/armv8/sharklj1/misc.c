...
void scx35_pmu_reconfig(void)
{
	 ...
	 */
	CHIP_REG_AND(REG_PMU_APB_PD_MM_TOP_CFG,
				~(BIT_PMU_APB_PD_MM_TOP_FORCE_SHUTDOWN |
				BIT_PMU_APB_PD_MM_TOP_AUTO_SHUTDOWN_EN));
	CHIP_REG_AND(REG_PMU_APB_PD_GPU_TOP_CFG,
				~(BIT_PMU_APB_PD_GPU_TOP_FORCE_SHUTDOWN |
				BIT_PMU_APB_PD_GPU_TOP_AUTO_SHUTDOWN_EN));

	CHIP_REG_OR(REG_AON_APB_APB_EB0, BIT_AON_APB_GPU_EB | BIT_AON_APB_MM_EB);

	CHIP_REG_OR(REG_MM_AHB_AHB_EB, BIT_MM_AHB_CKG_EB);
	CHIP_REG_OR(REG_MM_CLK_CORE_CGM_MM_AHB_CFG, 0x3);
}
#else
void scx35_pmu_reconfig(void) {}
#endif
...
void set_smt(void)
{
	...
	debugf("sharklj1: smt config: = 0x%x\n",__raw_readl(JVT_MT_CFG));
}
