...
int timer_init(void)
{
	...
	writel(readl(REG_AON_APB_APB_EB0) | BIT_AON_TMR_EB, REG_AON_APB_APB_EB0);
	writel(readl(REG_AON_APB_APB_RTC_EB) | BIT_AON_TMR_RTC_EB, REG_AON_APB_APB_RTC_EB);

	writel(readl(REG_AON_APB_APB_EB0) | BIT_AON_SYST_EB | BIT_AP_TMR0_EB, 
				REG_AON_APB_APB_EB0);
	writel(readl(REG_AON_APB_APB_RTC_EB) | BIT_AON_SYST_RTC_EB
				| BIT_AP_TMR0_RTC_EB, REG_AON_APB_APB_RTC_EB);

	writel(readl(REG_AON_APB_APB_EB1) | BIT_AP_TMR1_EB | BIT_AP_TMR2_EB,
				REG_AON_APB_APB_EB1);
	writel(readl(REG_AON_APB_APB_RTC_EB) | BIT_AP_TMR1_RTC_EB
				| BIT_AP_TMR2_RTC_EB, REG_AON_APB_APB_RTC_EB);
	/*init arch timer*/
	writel(0x1, 0x40400000);
	return 0;
}
...
{
	ulong now = readl(SYS_VALUE_SHDW);
