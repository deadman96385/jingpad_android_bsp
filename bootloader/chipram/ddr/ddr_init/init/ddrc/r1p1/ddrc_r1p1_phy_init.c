...
//	reg_bits_set(tmp_addr + 0x9*4, 4, 3, 0x0);//odtp off
	...
	/* freq < 1024 drv 60ohm */
	if(freq_sel < 4)
		reg_bits_set(tmp_addr + 0x10*4, 0, 16, 0x2020);//odtn 80ohm
	else
		reg_bits_set(tmp_addr + 0x10*4, 0, 16, 0x3030);//odtn 80ohm
...
//	ddrc_print_debug("rank0 training result: \r\n");
	...
	if ((wr_bit >> 14) & 0x1)//WVREF
	{
		...
		reg_bits_set(tmp_addr + 0xf*4, 24, 6, read_data);
		...
		{
			/*phy0 vref*/
			data_tmp = ((lpddr4_mr_info.p_odt_cfg + phy_train.freq_sel)->mr14) & 0x3f;
			/*dfs has only one channel vref, so use two channel average vref*/
			read_data = (data_tmp + read_data) / 2;
			lpddr_dmc_mrw_chn_sel(CMD_CHN_1);
		}
		(lpddr4_mr_info.p_odt_cfg + phy_train.freq_sel)->mr14 = u32_bits_set((lpddr4_mr_info.p_odt_cfg + phy_train.freq_sel)->mr14, 0, 6, read_data);
		if(0 == phy_train.rank)
		{
...
void phy_load_rw_perbit_reg(uint32 wr_bit, uint32 phy_base, uint32 freq, uint32 rank)
{
	...
	max_fdl = 0;
	if (((wr_bit >>9) & 0x1)) //RPERB dm use dq0 read perbit
	{
...
//		}
		...
		/*use read dm perbit training result*/
...
//		}
		...
		*/
		...
		data_tmp = __raw_readl(phy_base + 0xf2*4);
		data_tmp = (data_tmp >> 8) & 0xff;
		/*read will training second time use this litter dm*/
		if(phy_base == DMC_GUCPHY0_BASE)
		{
			if(read_dm_phy0 == 0)
				read_dm_phy0 = data_tmp;
			if(read_dm_phy0 < data_tmp)
				data_tmp = read_dm_phy0;
		}
		else
		{
			if(read_dm_phy1 == 0)
				read_dm_phy1 = data_tmp;
			if(read_dm_phy1 < data_tmp)
				data_tmp = read_dm_phy1;
		}
		if(data_tmp > 0x3a)
			data_tmp -= 5;
		else if(data_tmp > 0x35)
			data_tmp -= 4;
		else if(data_tmp > 0x32)
			data_tmp = 0x32;
		data_tmp1 = u32_bits_set(data_tmp1, 8, 8, (data_tmp & 0xff ) );
...
void ddrc_phy_training_conditions(uint32 freq_sel, uint32 rank)
{
	...
	reg_bits_set(0x310012e0, 0, 8, 0x0);
	reg_bits_set(0x310022e0, 0, 8, 0x0);
	reg_bits_set(0x310012e8, 16, 8, 0x0);
	reg_bits_set(0x310022e8, 16, 8, 0x0);
	reg_bits_set(0x310012F4, 0, 8, 0x0);
	reg_bits_set(0x310022F4, 0, 8, 0x0);
	reg_bits_set(0x310012FC, 16, 8, 0x0);
	reg_bits_set(0x310022FC, 16, 8, 0x0);
	if (0x8 == ddr_chip_cur.cs0_jedec_info->dw)
	{
//		ddrc_training_config_table[rank][freq_sel].cabt_enable = 0;
	}
	reg_bits_set(0x310012e0, 0, 8, 0x1f);
	reg_bits_set(0x310022e0, 0, 8, 0x1f);
	reg_bits_set(0x310012e8, 16, 8, 0x1f);
	reg_bits_set(0x310022e8, 16, 8, 0x1f);
	reg_bits_set(0x310012F4, 0, 8, 0x38);
	reg_bits_set(0x310022F4, 0, 8, 0x38);
	reg_bits_set(0x310012FC, 16, 8, 0x38);
	reg_bits_set(0x310022FC, 16, 8, 0x38);
}
...
void dll_half_mode_cfg(uint32 phy_base, uint32 freq_sel)
{
	uint32 read_data;
	uint32 data_tmp;
	uint32 wr_data;
	...
	if(regval > 0x80)
	{
		wr_data = (regval - 0x80) * 2;
		data_tmp = __raw_readl(tmp_addr + 0x1*4);
		data_tmp = u32_bits_set(data_tmp, 30, 2, 3);//31:dll half mode enable, 30:falling edge write.
		__raw_writel(tmp_addr + 0x1*4, data_tmp);
	}
	else
	{
		wr_data = regval * 2;
		data_tmp = __raw_readl(tmp_addr + 0x1*4);
		data_tmp = u32_bits_set(data_tmp, 30, 2, 2);//31:dll half mode enable, 30:falling edge write.
		__raw_writel(tmp_addr + 0x1*4, data_tmp);
	}
	...
	if(regval > 0x80)
	{
		wr_data = (regval - 0x80) * 2;
		data_tmp = __raw_readl(tmp_addr + 0x1*4);
		data_tmp = u32_bits_set(data_tmp, 29, 1, 1);//29:ca dll half mode
		__raw_writel(tmp_addr + 0x1*4, data_tmp);
	}
	else
	{
		wr_data = regval * 2;
		data_tmp = __raw_readl(tmp_addr + 0x1*4);
		data_tmp = u32_bits_set(data_tmp, 29, 1, 0);//29:ca dll half mode
		__raw_writel(tmp_addr + 0x1*4, data_tmp);
	}
	...
	{
		wr_data = (regval - 0x80) * 2;
		read_data = u32_bits_set(read_data, 0, 8, (read_data & 0xFF) + 1);
	}
	else
	{
		wr_data = regval * 2;
	}
	read_data = u32_bits_set(read_data, 16, 8, wr_data);
	regval = ((read_data >> 0x18) & 0xff);
	if(regval >= 0x80)
	{
		wr_data = (regval - 0x80) * 2;
		read_data = u32_bits_set(read_data, 8, 8, ((read_data >> 0x8) & 0xFF) + 1);
	}
	else
	{
		wr_data = regval * 2;
	}
	read_data = u32_bits_set(read_data, 24, 8, wr_data);
	__raw_writel(tmp_addr + 0xD*4, read_data);
}
...
//			phy_rwvref_seq(freq_sel, 1, rank);
	...
	{
		dll_half_mode_cfg(DMC_GUCPHY0_BASE, freq_sel);
		dll_half_mode_cfg(DMC_GUCPHY1_BASE, freq_sel);
//		master_dll_lock_seq_cut();
	}
...
void one_freq_point_init_flow_cut(void)
{
	char s_tmp[11] = {0};
	uint32 freq_sel = phy_train.freq_sel;
	uint32 this_freq = find_ddr_freq(freq_sel);
...
//	}
	...
	if(5 == freq_sel)
	{
		...
		dram_init_mrw_11_22(6);//freq_sel 6.  1536M
...
{
	uint32 i, max_freq_tmp;

#ifdef DDR_SCAN_ENABLE
	scan_freq = ddr_clk;
#endif
	max_freq = find_freq_num(ddr_clk);
	max_freq_tmp = max_freq;
