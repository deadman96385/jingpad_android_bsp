...
#else
#define UART_TX_BUF_ADDR UART1_TX_BUF_ADDR
#endif
#define UART_TX_BUF_CNT ((REG32(UART_TX_BUF_ADDR + 0xc)>>8)&0xff)


static void ddrc_print_str(const char *string)
{
	char *s1 = NULL;
#ifdef DDR_SCAN_ENABLE
	if(uart_log_disable != 0)
		return;
#endif
	s1 = (char *)string;
...
{
	return itoa_simple(num, itoc_tmp, 10);
}
...
/***ddrc_print_debug_location example***/
	...
	ddrc_print_debug(" \r\n");
#endif
void hex_to_str(char * str, uint32 val)
{
	uint32 tmp;
	int i;
	for(i = 0; i < 8; i++)
...
void reg_dump(u32 start_addr, u32 count)
{
	uint32 val, reg, i, j;
	char str[12], tmp;
