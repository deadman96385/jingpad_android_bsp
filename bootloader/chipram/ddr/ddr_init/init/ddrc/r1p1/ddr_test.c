...
void ddr_mem_test(void)
{
	...
	while(1)
	{
		...
		for ( i = 0; i < MEM_TEST_LEN; i+= 1024)
		{
			...
			if (regval)
			{
				ddrc_print_debug("little ddr copy failed\n");
				while(1);
			}
			else
			{
				//ddrc_print_debug("ddr copy pass\n");
			}
		}
		...
		if (regval)
		{
			ddrc_print_debug("big ddr cmp failed\n");
			while(1);
		}
		else
		{
			ddrc_print_debug("big mem cmp pass\n");
		}
	}
}
#else
...
int ddr_scan_fdl1_test(int start, int end)
{
	...
	ddrc_print_debug("bist scan start\r\n");
	...
	ddrc_print_debug("bist scan end\r\n");
	return 0;
}
...
void ddr_write_read_test(void)
{
	...
	while(1)
	{
		if(count > (1<<10))
			break;
		ddrc_print_debug("self refresh wait begin...\r\n");
		dmc_sprd_delay(500000*count);
		count *= 2;
		ddrc_print_debug("count:");
		ddrc_print_debug(itoa_simple(count, log_string, 16));
		ddrc_print_debug("\r\n");
		ddrc_print_debug("self refresh wait end, data check\r\n");
#endif
		for(i = 0; i < cs_size; i += 0x4)
		{
			if(*(volatile u32*)(DDR_START_ADDR_PHY + i) != (DDR_START_ADDR_PHY + i))
			{
				while(1)
				{
					ddrc_print_debug("address:");
					ddrc_print_debug(itoa_simple((DDR_START_ADDR_PHY + i + 1), log_string, 16));
					ddrc_print_debug("data:");
					ddrc_print_debug(itoa_simple(*(volatile u32*)(DDR_START_ADDR_PHY + i), log_string, 16));
					ddrc_print_debug("\r\n");
					ddrc_print_debug("ddr write read test fail\r\n");
...
int ddr_selfrefresh_test(u32 timeout)
{
	...
	for(i = 0; i < count ; i++)
	{
		...
		ddrc_print_debug(itoa_simple((u32)start_base), log_string, 16);
		ddrc_print_debug(itoa_simple(((u32)start_base+sr_length)), log_string, 16);
