...
extern DDRC_FREQ_INFO_T  * freq_info;
...
#endif
//This format is Shrinking eg:"0x800<<16 = 1Gb"
...
int dram_revision_info_update(u32 cs_num)
{
	...
	if(DRAM_CS_0 == cs_num)
	{
		dram_info.mr_reg_cs0 &= ~(0xffffff);
		dram_info.mr_reg_cs0 |= ((mr7_val&0xff)<<16)|((mr6_val&0xff)<<8)|((mr5_val&0xff)<<0);
	}else
	{
		dram_info.mr_reg_cs1 &= ~(0xffffff);
		dram_info.mr_reg_cs1 |= ((mr7_val&0xff)<<16)|((mr6_val&0xff)<<8)|((mr5_val&0xff)<<0);
	}
...
void dram_byte_latency_mode()
{
	...
	for(cs_num=0;cs_num<2;cs_num++)
	{
		...
		if((mr0_val>>1)&0x1)
		{
			reg_bit_set(DMC_CTL0_(0x10c),16,8,0xff);
...
u32 mr8_to_detect_info(u32 mr8_value,u32 cs_num)
{
	...
	if(dram_info.dram_type==DRAM_LP3)
	{
		...
		if(mr8_width == WIDTH_X16)//LPDDR3:32bit  consist 2die so *2
		{
			mem_size *=2;
		}
	}else
	{
		mr8_width += WIDTH_X16; //In this position 0:X32 1:X16 2:X8 for per die,If read 0 for mr8[5:2],should transform this format
		for(i=0; i< sizeof(LPDDR4_mr8_size)/sizeof(LPDDR4_mr8_size[0]);i++)
		{
			if(mr8_size == LPDDR4_mr8_size[i].mr8_size)
			{
				mem_size=(LPDDR4_mr8_size[i].mem_size*2);//LPDDR4 or 4x have two channel
			}
		}
	...
	//Put Size and Width information into one variable
	//31:24<DDR Type> CS1 :{23:16<SIZE> ,15:12<Width>} CS0:{ 11:4<SIZE> ,3:0<Width>}
	...
	//Update cs size in Global Variable
	if(cs_num==DRAM_CS_0)
	{
...
void dram_type_pinmux_auto_detect()
{
	...
	*/
	...
	{
	case DRAM_LP3:
		dram_info.pinmux_type=LPDDR3_PINMUX_CASE1;
		...
		reg_bit_set(DDRPHY_VREF, 2, 8,0x50);//phy_vref_adj,This vref only use for lpddr3
		//set dfs para-lp3
		freq_info=(DDRC_FREQ_INFO_T *)(&ddrc_freq_info_lp3[0]);
		break;
	case DRAM_LP4:
		...
		reg_bit_set(DDRPHY_VREF, 1, 1,0x1);//phy_vref_pd ,Enable power down and save power Because this vref use for LPDDR3
		//set dfs para-lp4
		freq_info=(DDRC_FREQ_INFO_T *)(&ddrc_freq_info_lp4[0]);
		break;
	case DRAM_LP4X:
		dram_info.dram_detect_type |= (DRAM_LP4<<24);
		dram_info.pinmux_type=LPDDR4_PINMUX_CASE0;
		...
		reg_bit_set(DDRPHY_VREF, 1, 1,0x1);//phy_vref_pd,Same as above
		//set dfs para-lp4
		freq_info=(DDRC_FREQ_INFO_T *)(&ddrc_freq_info_lp4[0]);
		break;
	}
...
void dram_size_auto_detect()
{
	...
	ret=mr8_to_detect_info(cs_mr8_val,DRAM_CS_0);//Caculate DRAM size and update Global variable dram_info.dram_detect_type
	if(!ret)
	{
#if TEST_DEBUG_LOG_LEVEL>1
	...
	/*According above information get dram setting or Manual set dram setting*/
...
/*According MR5,MR6,MR7(Manufacturer ID) limit Training Top freq*/
void dram_freq_auto_detect(u32* target_fn)
{
#ifdef DRAM_FREQ_AUTO_DETECT
	u32 fn=0;
	u32 fixed_mr_val=dram_info.mr_reg_cs0 & (~(0xff<<24));
	//*target_fn=DRAM_TOP_FREQ;//#define in project include file
...
/*According variable ddr_mode get Training high point */
/*Start freq:bit8*/
u32 get_training_high_point(u32 ddr_training_flag)
{
	int fn=0;
	if((dram_info.dram_type==DRAM_LP4)||(dram_info.dram_type==DRAM_LP4X))
		...
		for(fn=7;fn>=0;fn--)
			{
			if(((ddr_training_flag>>(8+fn))&0x01)==0)
				break;
			}
		if(fn<0) fn=0;
		switch (fn)
			{
				...
				case 0:return 256;
				default:
					//dmc_print_str("get_training_high_point Error!");
					while(1);
			}
	}
...
{
	(phy_tmg+fn)->cfg_io_ds_cfg &= ~(0x7<<8);//rf_phy_io_ds_dq_odtp--clear p
	(phy_tmg+fn)->cfg_io_ds_cfg &= ~(0x7<<12);//rf_phy_io_ds_dq_odtn
...
{
	//#define DMC_LP4_xxxx_DTMG11_LP4X	0x0b550b55 => 0B:MR11
	//This set value become effective after dfs  this point
	//DFS flow in training flow
	(dmc_dtmg+fn)->dmc_dtmg11 &= ~(0x7<<16);
	(dmc_dtmg+fn)->dmc_dtmg11 |= (odt_val<<16);

	(dmc_dtmg+fn)->dmc_dtmg11 &= ~(0x7<<0x0);
	(dmc_dtmg+fn)->dmc_dtmg11 |= (odt_val<<0x0);

	__raw_writel(DMC_CTL0_(0x01ac+fn*0x60),(dmc_dtmg+fn)->dmc_dtmg11);
}

void dram_ca_odt_set(u32 fn,u32 odt_val)
{
	//This set value become effective after dfs  this point
	(dmc_dtmg+fn)->dmc_dtmg11 &= ~(0x7<<4);
	(dmc_dtmg+fn)->dmc_dtmg11 |= (odt_val<<4);

	(dmc_dtmg+fn)->dmc_dtmg11 &= ~(0x7<<20);
	(dmc_dtmg+fn)->dmc_dtmg11 |= (odt_val<<20);
