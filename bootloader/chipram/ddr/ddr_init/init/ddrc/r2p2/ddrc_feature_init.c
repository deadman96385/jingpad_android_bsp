...
void ctrl_urgent_set()
{
	...
	__raw_writel(DMC_CTL0_(0x130),0x80804040);
	/*
	//high urgent
...
{
	//bypass qosc urgent
	reg_bit_set(QOS_SWITCH,0,2,0x3);//qos_urgent_sel
	/*
	...
	*/
	...
	__raw_writel(QOS_THRESHOLD_1,0x66);
}

void ddrc_ctrl_perf_set()
{
	//performace
	reg_bit_set(DMC_CTL0_(0x0010), 0,20,0x90001);//rf_port_prefetch_en
	/*
	...
	*/
	reg_bit_set(DMC_CTL0_(0x0024), 8, 1,0x0);//rf_urgent_rd_ch0
...
void ap_ctrl_qos_set()
{
	...
	__raw_writel(APCPU_QOS_CTRL,0x00ff66);
}
