...
static uint32 bist_fail_num[3] = {0};
...
static uint32 dmc_ddr_test(u32 bist_num,u32 addr)
{
	volatile uint32 i=0;
	uint32 test_num=0;
	uint64 chip_size;
...
{
	u32 data_tmp;
	data_tmp = reg_bits_get(CTRL_REG_BASE + 0x0124, 24, 2);
	if ( data_tmp != 0x3)
	{
...
void ddr_offline_scan_parameter(scan_type_parameter type_para,u16 vref_val)
{
	...
	for (i=0; i<=temp_parameter.scan_num; i=i+X_INCREASE_STEP)
	{
		...
		{
			case 0:
				phy_delay_line_set_wr(temp_parameter.dmc_phy_base,i,freq_sel,temp_parameter.data_module);
				break;
			case 1:
				phy_delay_line_set_rd(temp_parameter.dmc_phy_base,i,freq_sel,temp_parameter.data_module);
				break;
			default:
	...
	for (; i<=temp_parameter.scan_num;)
	{
		...
		if(ret)
		{
			if(store_reboot_value->window_fail_flag==0)//Judge all file in line
			{
				...
				//_start();
				ddr_init();
				REG32(CTRL_REG_BASE + 0x0124)=0;
				reg_bits_set(CTRL_REG_BASE + 0x010c,12,1,0);
				udelay(5);
...
void ddr_offline_scan_wr(u16 max ,u16 min,best_para*pPara, u32 rank)
{
	u8 i=0,j;
	int vref_lv=0;
	//reg_bits_set(PHY_VREFE_ADDRESS,0,1,0); //phy_vref_hi_c
	//reg_bits_set(PHY_VREFE_ADDRESS,1,1,0); //phy_vref_pd
	ddrc_print_scan("\r\noff line scan write\r\n");
	dmc_ctrl_reg_unlock();
	for(i=0;i<(sizeof(phy_scan_para_wr)/sizeof(scan_type_parameter));i++)
	{
		...
		bist_start_address = 0;//get_bist_address_lp4(i/2,rank);
...
void ddr_offline_scan_ca(u16 vref_max ,u16 vref_min)
{
	...
	reg_bits_set(CTRL_REG_BASE + 0x010c,12,1,1);
}
...
void ddr_offline_scan_seq(int rank)
{
	...
	u32 i;
	reg_bits_set(0x640100b0,8,1,0);//Enable address 0x800000 space
	reg_bits_set(0x64010544,5,1,0);
	...
	if(store_reboot_value->read_write_end_flag==0)//Avoid scan ca repeat call read & write function
	{
		...
		ddr_offline_scan_rd(VREFI_OP_MAX,VREFI_OP_MIN,&read_op_para,rank);
	...
	//	back_register_0=REG32(PHY_VREFE_ADDRESS);
		...
		ddr_offline_scan_wr(VREFE_DAC_MAX,VREFE_DAC_MIN,&write_dac_para,rank);
...
void ddr_scan_vdd_seq(void)
{
	...
	REG32(CTRL_REG_BASE + 0x0124)=0;
	reg_bits_set(CTRL_REG_BASE + 0x010c,12,1,1);
	freq_sel = (REG32(CTRL_REG_BASE + 0x012c) >> 8) & 0x7;//get freq for register
