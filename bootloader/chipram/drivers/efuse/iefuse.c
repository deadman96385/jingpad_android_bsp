...
static void ap_efuse_reset(void)
{
	...
	CHIP_REG_OR(REG_AON_APB_APB_RST0, BIT_AON_APB_EFUSE_SOFT_RST);
	udelay(10);
	CHIP_REG_AND(REG_AON_APB_APB_RST0, (~BIT_AON_APB_EFUSE_SOFT_RST));
}
...
{
	CHIP_REG_OR(REG_AON_APB_APB_EB0, BIT_AON_APB_EFUSE_EB);
}

static void ap_efuse_disable(void)
{
	CHIP_REG_AND(REG_AON_APB_APB_EB0, (~BIT_AON_APB_EFUSE_EB));
}
...
u32 ddie_efuse_read(int blk_index)
{
       return sprd_ap_efuse_read(blk_index);
}
