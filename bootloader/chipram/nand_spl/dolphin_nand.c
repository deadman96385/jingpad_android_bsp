...
STATIC_FUNC int sprd_dolphin_nand_read_lp(struct mtd_info *mtd,uint8_t *mbuf, uint8_t *sbuf,uint32_t raw)
{
	...
	if(mbuf) {
		sprd_memcpy(mbuf, (const void *)dolphin->v_mbuf, dolphin->write_size);
	}
	if(sbuf) {
		sprd_memcpy(sbuf, (const void *)dolphin->v_oob, dolphin->oob_size);
	}
...
STATIC_FUNC int sprd_dolphin_nand_write_lp(struct mtd_info *mtd,const uint8_t *mbuf, uint8_t *sbuf,uint32_t raw)
{
	...
	if(mbuf) {
		sprd_memcpy((void *)dolphin->v_mbuf, (const void *)mbuf, dolphin->write_size);
	}
	if(sbuf) {
		sprd_memcpy((void *)dolphin->v_oob, (const void *)sbuf, dolphin->oob_size);
	}
...
STATIC_FUNC int sprd_nand_probe(struct platform_device *pdev)
{
	...
	sprd_memset(&g_dolphin, 0 , sizeof(g_dolphin));
	...
	sprd_memset((char *)sprd_mtd, 0, sizeof(struct mtd_info));
	sprd_memset((char *)this, 0, sizeof(struct nand_chip));
