...
/*
extern void Dcache_InvalRegion(uint32_t, uint32_t);
extern void Dcache_CleanRegion(uint32_t, uint32_t);
*/
...
static void usb3_pcd_ep0_out_start(usb3_pcd_t *pcd)
{
	...
	// Dcache_CleanRegion((uint32_t)desc, sizeof(*desc));
...
static void usb3_handle_ep0_xfer(usb3_pcd_t *pcd, uint32_t event)
{
	if (pcd->ep0state == EP0_IDLE) {
		// Dcache_InvalRegion((uint32_t)usb3_ep0_setup_pkt(pcd), 8);
		do_setup(pcd);
	} else {
                                 ...
                                 usb3_pcd_ep_t *ep, uint32_t event)
{
	if (ep == pcd->out_ep) {
		...
		// Dcache_InvalRegion((uint32_t)desc, sizeof(*desc));
		g_out_len = sizeof(g_out_buffer) -
		            (desc->status >> DSCSTS_XFRCNT_SHIFT
		             & DSCSTS_XFRCNT_BITS >> DSCSTS_XFRCNT_SHIFT);
		/* Sync transmit buffer */
		/*
		Dcache_InvalRegion((uint32_t)g_out_buffer,
		                   sizeof(g_out_buffer));
		 */
...
static uint32_t get_eventbuf_event(usb3_device_t *dev, int size)
{
	...
	/* Update current event ptr */
	*(phys_addr_t*)(dev->event_buf + dev->event_size) =
	        (phys_addr_t)dev->event_ptr;
...
/**
 * This routine flushes any present events
 */
 ...
 */
static void usb3_init_eventbuf(usb3_device_t *dev)
{
...
static void usb_receive(usb3_pcd_t *pcd)
{
	...
	/* Dcache_CleanRegion((uint32_t)ep->ep.dma_desc,
	                   sizeof (usb3_dma_desc_t));
        */
 ...
 */
void usb_send(unsigned char *buf, int len)
{
	...
	/* Sync transmit buffer */
	// Dcache_CleanRegion((uint32_t)buf, len);
 ...
 */
int usb_get_packet(uint8_t* buf, int len)
{
	usb3_pcd_t *pcd = &g_usb3_dev.pcd;
 ...
 * Note: Since USB is initialized and enumerated in ROM code,
  ...
  * in this stage (FDL1), and some properties would be got from USB registers.
 */
static void usb3_driver_init(unsigned long reg_base_addr)
{
