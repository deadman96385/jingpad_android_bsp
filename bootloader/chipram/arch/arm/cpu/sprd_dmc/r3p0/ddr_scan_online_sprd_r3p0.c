...
void set_half_mode(u32 half_mode_en)
{
	DMC_R3P0_REG_INFO_PTR pdmc = (DMC_R3P0_REG_INFO_PTR)PUB_DMC_BASE;
	u32 regval, idx;
	u32 sts_val[5];
	...
	idx = (pdmc->dmc_dcfg11>> 4) & 0x3; /* drf_dfs_reg_sel */
	regval = pdmc->dmc_dtmg_f[idx][11];
	regval = u32_bits_set(regval, 8, 1, half_mode_en);
	pdmc->dmc_dtmg_f[idx][11] = regval;
}
/*
 * init the quart delay line cnt of half ddr clk
 */
void ui_qtr_dly_cnt_init(void)
{
	...
	u32 half_mode_en;
	u32 i;
	DMC_R3P0_REG_INFO_PTR pdmc = (DMC_R3P0_REG_INFO_PTR)DMC_REG_ADDR_BASE_PHY;

	/*
	* beacause half_mode the rfdl_cnt register value is minus than the right rfdl_cnt value
	* so when we want to get the rfdl_cnt, we need to close half mode
	*/
	half_mode_en = get_half_mode();
	set_half_mode(0);
	dmc_sprd_delay(100);
	dmc_dll_init(DDR_DLL_CFG);
	dmc_sprd_delay(100);
	ca_rfdl_cnt = pdmc->dmc_sts_dll_ac & 0x7f;
	ds_rfdl_cnt[0] = pdmc->dmc_sts_dll_ds0 & 0x7f;
	ds_rfdl_cnt[1] = pdmc->dmc_sts_dll_ds1 & 0x7f;
	ds_rfdl_cnt[2] = pdmc->dmc_sts_dll_ds2 & 0x7f;
	ds_rfdl_cnt[3] = pdmc->dmc_sts_dll_ds3 & 0x7f;

	if(get_clk_mode() == 1)
	{
		ca_rfdl_cnt = ca_rfdl_cnt >> 1;
		ds_rfdl_cnt[0] = ds_rfdl_cnt[0] >> 1;
		ds_rfdl_cnt[1] = ds_rfdl_cnt[1] >> 1;
		ds_rfdl_cnt[2] = ds_rfdl_cnt[2] >> 1;
		ds_rfdl_cnt[3] = ds_rfdl_cnt[3] >> 1;
	}
	for(i = 0; i < 4; i++)
	{
		ds_ui_qrt_dly[i] = ds_rfdl_cnt[i] << 2;
	}
	ca_ui_qrt_dly = ca_rfdl_cnt << 2;
	set_half_mode(half_mode_en);
	print_String("ui_qtr_dly_cnt_init: ca_rfdl_cnt, ds_rfdl_cnt[0], ds_rfdl_cnt[1], ds_rfdl_cnt[2], ds_rfdl_cnt[3]\r\n");
	print_Dec(ca_rfdl_cnt);
	print_Dec(ds_rfdl_cnt[0]);
	print_Dec(ds_rfdl_cnt[1]);
	print_Dec(ds_rfdl_cnt[2]);
	print_Hex(ds_rfdl_cnt[3]);
	print_String("\r\n");
}
/* Fast bist : keep register as previous value set .... just trigger bist to save time ..*/
static int fast_sipi_bist_test(void)
{
	u32 regval;
	u32 len;
	int tout;
...
static int sipi_bist_simple_test(void)
{
...
#else
	u32 cs0_size,cs1_size;
	dmc_print_str("ddr all bist test start\r\n");
	...
	if (ddr_chip_cur.cs_num == 2)
	{
		...
		if (BIST_OK != dmc_bist_test(BIST_TYPE_SIPI, &para))
		{
			dmc_print_str("cs1 bist failed\r\n");
			return -1;
		}
	}
	dmc_print_str("ddr all bist test end\r\n");
	return 0;
#endif
...
int set_ca_dll_with_offset(s32 bound_ok[2][2], u32 default_clkwr_dll_ac)
{
	...
	if ((get_clk_mode() == 0)&&(get_half_mode() == 0))
	{
		rfdll_cnt >>= 1;
	}
	if ((get_clk_mode() == 1)&&(get_half_mode() == 0))
	{
		rfdll_cnt >>= 2;
	}
	if ((get_clk_mode() == 1)&&(get_half_mode() == 1))
	{
		rfdll_cnt >>= 1;
	}
#else
...
#endif
void clk_right_shift(u32 qtr_dly)
{
	u32 regval;
	DMC_R3P0_REG_INFO_PTR pdmc = (DMC_R3P0_REG_INFO_PTR)PUB_DMC_BASE;
	if(qtr_dly > 0x1f)
	...
	regval = u32_bits_set(regval, 24, 5, qtr_dly);
...
int ca_training_main_delay(int phase2, s32 delay[2])
{
	...
	scan_steps = (ca_ui_qrt_dly + 0x20) >> 2;
	clk_right_shift(0x1f);
	...
	do {
...
#ifdef DMC_CA_SWAP
                temp = phy_rcv_data & 0xffff;
#ifndef MANUAL_CONFIG_SWAP
		if((ddr_chip_cur.pcb_swap_type == INVALID_T) || (ddr_chip_cur.pcb_swap_type == EMCP_LP3_T) || (ddr_chip_cur.pcb_swap_type == LP2_DIE_LP3_SWAP_EMCP_LP3_T))
		{
			phy_rcv_data = (((temp  & 0x8000)>> 15 )<<14)| //DQ[14]<---bit[15]
			    (((temp & 0x4000)>> 14)<<12)| //DQ[12]<-----bit[14]
			    (((temp & 0x2000)>> 13)<<15)| //DQ[15]<-----bit[13]
			    (((temp & 0x1000)>> 12)<<13)| //DQ[13]<-----bit[12]
			    (((temp & 0x0f00)>> 8)<<8)  | //DQ[11:8]<---bit[11:8]
                            (temp   & 0x0080) |           //DQ[7]<------bit[7]
			    (((temp & 0x0040)>> 6)<<4)  | //DQ[4]<------bit[6]
			    (((temp & 0x0030)>> 4)<< 0) | //DQ[1:0]<----bit[5:4]
			    (((temp & 0x0008)>> 3)<<5)  | //DQ[5]<------bit[3]
			    (((temp & 0x0004)>> 2)<<6)  | //DQ[6]<------bit[2]
			    (((temp & 0x0002)>> 1)<<2)  | //DQ[2]<------bit[1]
			    (((temp & 0x0001)>> 0)<<3);   //DQ[3]<------bit[0]
		}
		if((ddr_chip_cur.pcb_swap_type == EMCP_LP2_T) || (ddr_chip_cur.pcb_swap_type == LP3_DIE_LP2_SWAP_EMCP_LP2_T))
		{
			phy_rcv_data = (((temp  & 0x8000)>> 15 )<<15)| //DQ[14]<---bit[15]
			    (((temp & 0x4000)>> 14)<<12)| //DQ[12]<-----bit[14]
			    (((temp & 0x2000)>> 13)<<13)| //DQ[13]<-----bit[13]
			    (((temp & 0x1000)>> 12)<<14)| //DQ[14]<-----bit[12]
			    (((temp & 0x0800)>> 11)<<11)| //DQ[11]<-----bit[11]
			    (((temp & 0x0400)>> 10)<<9)| //DQ[9]<-----bit[10]
			    (((temp & 0x0200)>> 9)<<8)| //DQ[8]<-----bit[9]
			    (((temp & 0x0100)>> 8)<<10)| //DQ[10]<-----bit[8]
			    (((temp & 0x0080)>> 7)<<6)  | //DQ[6]<------bit[7]
			    (((temp & 0x0040)>> 6)<<3)  | //DQ[3]<------bit[6]
			    (((temp & 0x0020)>> 5)<<1) | //DQ[1]<----bit[5]
			    (((temp & 0x0010)>> 4)<<0) | //DQ[0]<----bit[4]
			    (((temp & 0x0008)>> 3)<<5)  | //DQ[5]<------bit[3]
			    (((temp & 0x0004)>> 2)<<7)  | //DQ[7]<------bit[2]
			    (((temp & 0x0002)>> 1)<<2)  | //DQ[2]<------bit[1]
			    (((temp & 0x0001)>> 0)<<4);   //DQ[4]<------bit[0]
		}
		if((ddr_chip_cur.pcb_swap_type == DISCRETE_LP3_T) || (ddr_chip_cur.pcb_swap_type == LP2_DIE_LP3_SWAP_DISCRETE_LP3_T))
		{
			phy_rcv_data = (((temp  & 0x8000)>> 15 )<<12)| //DQ[12]<---bit[15]
			    (((temp & 0x4000)>> 14)<<11)| //DQ[11]<-----bit[14]
			    (((temp & 0x2000)>> 13)<<15)| //DQ[15]<-----bit[13]
			    (((temp & 0x1000)>> 12)<<14)| //DQ[14]<-----bit[12]
			    (((temp & 0x0800)>> 11)<<13)| //DQ[13]<-----bit[11]
			    (((temp & 0x0400)>> 10)<<10)| //DQ[10]<-----bit[10]
			    (((temp & 0x0200)>> 9)<<8)| //DQ[8]<-----bit[9]
			    (((temp & 0x0100)>> 8)<<9)| //DQ[9]<-----bit[8]
			    (((temp & 0x0080)>> 7)<<5)  | //DQ[5]<------bit[7]
			    (((temp & 0x0040)>> 6)<<7)  | //DQ[7]<------bit[6]
			    (((temp & 0x0020)>> 5)<<0) | //DQ[0]<----bit[5]
			    (((temp & 0x0010)>> 4)<<3) | //DQ[3]<----bit[4]
			    (((temp & 0x0008)>> 3)<<4)  | //DQ[4]<------bit[3]
			    (((temp & 0x0004)>> 2)<<6)  | //DQ[6]<------bit[2]
			    (((temp & 0x0002)>> 1)<<2)  | //DQ[2]<------bit[1]
			    (((temp & 0x0001)>> 0)<<1);   //DQ[1]<------bit[0]
		}
#else
		phy_rcv_data = DDR_DQ_SWAP_MAP_BYTE(temp);
#endif
#endif

#ifdef CONFIG_SOC_PIKE2
		temp = phy_rcv_data & 0x0FFFF;
#ifndef MANUAL_CONFIG_SWAP
		if((ddr_chip_cur.pcb_swap_type == EMCP_LP2_T) || (ddr_chip_cur.pcb_swap_type == LP3_DIE_LP2_SWAP_EMCP_LP2_T))
		{
			phy_rcv_data = DDR_DQ_SWAP_MAP_BYTE_LP2(temp);
		}
		else if((ddr_chip_cur.pcb_swap_type == EMCP_LP3_T) || (ddr_chip_cur.pcb_swap_type == LP2_DIE_LP3_SWAP_DISCRETE_LP3_T))
		{
			phy_rcv_data = DDR_DQ_SWAP_MAP_BYTE_LP3(temp);
		}
		else
		{
			dmc_print_str("pcb swap type error\r\n");
		}
#else
		phy_rcv_data = DDR_DQ_SWAP_MAP_BYTE(temp);
#endif
		...
		{
			clk_right_shift(0);
			regval = u32_bits_set(regval, 0,  7, main_delay-8);
		}
		pdmc->dmc_clkwr_dll_ac = regval;
	}while(main_delay < scan_steps);
...
#endif
void dqs_right_shift(u32 qrt_dly)
{
	u32 regval;
	DMC_R3P0_REG_INFO_PTR pdmc = (DMC_R3P0_REG_INFO_PTR)PUB_DMC_BASE;
	if(qrt_dly > 0x1f)
	...
	regval = pdmc->dmc_dmdqs_inout_dl_ds0;
	regval = u32_bits_set(regval, 0, 5, qrt_dly);
	pdmc->dmc_dmdqs_inout_dl_ds0 = regval;
...
/* Write data eye training and skew
  ...
  */
int dmc_lpddr3_wde_training(void)
{
	DMC_R3P0_REG_INFO_PTR pdmc = (DMC_R3P0_REG_INFO_PTR)PUB_DMC_BASE;
	u32 default_bist_mask;
	...
	for(i = 0; i < 4; i++)
	{
		...
		print_Dec(i);
		print_String("\r\n");
		dqs_right_shift(0x1f);
		scan_steps = ((ds_ui_qrt_dly[i] >> 2) + 8);
		/* Step 2.  Search the pass window of  clkwr_dll_dsx*/
		for(j = MIN_DQ_DELAY; j < scan_steps;  j++)
		{
			if(j <  (scan_steps - 8))
			{
...
#else
int dmc_lpddr3_wde_training(void) { return 0}
#endif
...
#else
int dmc_lpddr3_write_leveling(void) {return 0;}
#endif
...
#ifdef CONFIG_FDL1
int ddr_bist_test(int start, int end)
{
	u64 chip_size;
	BIST_SIPI_PARA para;
	u32 bist_pattern_data[8] = {
