...
#include "ddr_scan_offline_sprd_r2p0.h"
...
static PROJECT_ID project_id_cur = PROJECT_SHARKLJ1;
#else
...
void scan_log_head(void)
{
	...
	regval = (1000000/clk_rate)/regval/2;
...
int  ddr_scan_offline_sprd_r2p0(void)
{
	get_scan_ddr_id();
	scan_log_head();
	print_log_start();
