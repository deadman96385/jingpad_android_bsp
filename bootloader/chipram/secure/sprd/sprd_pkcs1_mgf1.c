...
static uint8_t buf[SPRD_HASH_MAX_HASH_SIZE] __attribute__ ((aligned(8)));
static uint8_t tmp[SPRD_HASH_MAX_HASH_SIZE+4] __attribute__ ((aligned(8)));
/**
		...
		unsigned char *mask, unsigned long masklen)
{
	...
	sprd_pal_memset(buf, 0, SPRD_HASH_MAX_HASH_SIZE);
	sprd_pal_memset(tmp, 0, SPRD_HASH_MAX_HASH_SIZE+4);
	...
	while (masklen > 0) {
		...
		if ((err = sprd_crypto_do(hash_type, tmp, seedlen+4, buf,
						NULL, NULL, 0, NULL, 0, NULL, 0)) != SPRD_CRYPTO_SUCCESS) {
			goto LBL_ERR;
		}
#endif
