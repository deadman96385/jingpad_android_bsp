...
uint32_t sprd_pkcs1_pss_decode(const uint8_t *msghash, uint32_t msghashlen,
		...
		uint32_t saltlen, sprd_crypto_algo_t hash_type,
		...
		uint32_t modulus_bitlen, int *res)
{
	...
	*res = SPRD_VERIFY_FAILED;
	...
	if ((err = sprd_pkcs1_mgf1(mgf1_hash_type, hash, hLen,
					mask, modulus_len - hLen - 1)) != SPRD_CRYPTO_SUCCESS) {
		goto LBL_ERR;
	}
	...
	if ((err = sprd_crypto_do(hash_type, mask, 8+msghashlen+saltlen,
					mask, NULL, NULL, 0, NULL, 0, NULL, 0)) != SPRD_CRYPTO_SUCCESS) {
		goto LBL_ERR;
	}
#endif
