#include <security/sec_efuse_pike2.h>
#include <timer.h>
#include <asm/arch/chip_pike2/aon_sec_apb.h>
#include <asm/arch/chip_pike2/aon_apb.h>
#include <asm/arch/sprd_reg.h>

static inline void set_reg_bit(uint32 addr, uint32 bit)
{
	(*(volatile uint32 *)(addr)) |= 1<<bit;
}

static inline void clr_reg_bit(uint32 addr, uint32 bit)
{
	(*(volatile uint32 *)(addr)) &= BIT_MASK(bit);
}

static inline void write32(uint32 addr, uint32 val)
{
    *(volatile uint32_t *)addr = val;
}

static inline uint32_t read32(uint32 addr)
{
...
Efuse_Result_Ret efuse_huk_program(void)
{
	...
	set_reg_bit(REG_AP_AHB_CE_SEC_EB,12);			// enable ce module
	...
	clr_reg_bit(REG_AP_AHB_CE_SEC_EB,12); 			// disable ce module
