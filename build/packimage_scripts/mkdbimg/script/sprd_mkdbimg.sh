...
if [[ $type -eq 1 ]]; then
	$ROOTDIR/../bin/mkdbimg $IMAGE_FDL1 $CERT $KEY_PATH $SOCID $MASK $MASK $PSS_FLAG
elif [[ $type -eq 2 ]]; then
	$ROOTDIR/../bin/mkdbimg $IMAGE_FDL1 $CERT $KEY_PATH $UID0 $UID1 $MASK $PSS_FLAG
fi
	echo -e "\033[33m mkdbimg fdl1-sign.bin ok!\033[0m "
fi
if [ -f $IMAGE_SPL ] ;then
if [[ $type -eq 1 ]];then
	 $ROOTDIR/../bin/mkdbimg $IMAGE_SPL $CERT $KEY_PATH $SOCID $MASK $MASK $PSS_FLAG
elif [[ $type -eq 2 ]];then
	 $ROOTDIR/../bin/mkdbimg $IMAGE_SPL $CERT $KEY_PATH $UID0 $UID1 $MASK $PSS_FLAG
 fi
