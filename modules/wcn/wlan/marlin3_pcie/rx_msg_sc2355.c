...
int sprdwl_rx_init(struct sprdwl_intf *intf)
{
	...
	if (IS_ERR(rx_if->rx_thread)) {
          ret = PTR_ERR(rx_if->rx_thread);
          wl_err("%s SPRDWL_RX_THREAD create failed %d\n", __func__, ret);
          goto err_rx_work;
	}
	...
	if (IS_ERR(rx_if->rx_net_thread)) {
          ret = PTR_ERR(rx_if->rx_net_thread);
          wl_err("%s SPRDWL_RX_NET_QUEUE create failed %d\n", __func__, ret);
          goto err_rx_work;
	}
#endif
