...
#include "vdsp_spipe.h"

#ifdef pr_fmt
#undef pr_fmt
#endif
#define pr_fmt(fmt) "VDSP_SPIPE: %d %d %s : "\
...
static int spipe_open(struct inode *inode, struct file *filp)
{
	...
	if (vdsp_sbuf_status(spipe->init->dst, spipe->init->channel) != 0) {
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	pr_info("spipe_write\n");
	return vdsp_sbuf_write(sbuf->dst, sbuf->channel, sbuf->bufid,
			(void *)buf, count, timeout);
}
...
static unsigned int spipe_poll(struct file *filp, poll_table *wait)
{
	...
	return vdsp_sbuf_poll_wait(sbuf->dst, sbuf->channel, sbuf->bufid,
			filp, wait);
}
...
static int spipe_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node && !init) {
		...
		for_each_child_of_node(np, chd) {
			rval = spipe_parse_dt(&init, chd, &pdev->dev);
			if (rval) {
			...
			spipe = devm_kzalloc(&pdev->dev,
					     ...
					     GFP_KERNEL);
			if (spipe == NULL) {
				vdsp_sbuf_destroy(init->dst, init->channel);
				spipe_destroy_pdata(&init, &pdev->dev);
...
static int  spipe_remove(struct platform_device *pdev)
{
	...
	if (spipe) {
		...
		vdsp_sbuf_destroy(spipe->init->dst, spipe->init->channel);
...
static int __init vdsp_spipe_init(void)
{
	spipe_class = class_create(THIS_MODULE, "vdspspipe");
...
static void __exit vdsp_spipe_exit(void)
{
	class_destroy(spipe_class);
	platform_driver_unregister(&spipe_driver);
}

module_init(vdsp_spipe_init);
module_exit(vdsp_spipe_exit);
