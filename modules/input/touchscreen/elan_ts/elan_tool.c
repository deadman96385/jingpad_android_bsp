...
int rawdata_test_flag = 0;//use fail for default
...
//begin add by kun.zheng for task 10128739  davinci R on 2021/03/30
...
static ssize_t show_tp_module_test(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{

    if ((dev_mkdir("/storage/self/primary/rawdata", S_IRUGO | S_IWUSR)) != 0){
          printk("[elan] mkdir mp directory /sdcard/rawdata/ failed");
    }
...
static DEVICE_ATTR(tp_module_test, S_IRUGO | S_IWUSR, show_tp_module_test,  store_tp_module_test);
//end add by kun.zheng for task 10128739  davinci R on 2021/03/30
