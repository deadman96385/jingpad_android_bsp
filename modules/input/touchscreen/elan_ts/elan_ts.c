...
struct elan_ts_data *elan_ts;
static unsigned gesture_mode = 0;
static DECLARE_WAIT_QUEUE_HEAD(waiter);
const char *calibration_mode  = "cali";
static int calibration_mode_flag = 0;
static int get_cali_mode(char *cali_mode)
{
    ...
    if (!np){
        printk("Can't get the /chosen\n");
        return 0;
    }
    ret  = of_property_read_string(np, "bootargs", &cmd_line);
    if (ret < 0){
        printk("Can't get the bootargs\n");
    }
...
#ifdef GESTURE_MODE
static int elan_check_gesture(struct i2c_client *client)
{
    uint8_t hid_cmd[HID_CMD_LEN] = {0x04, 0x00, 0x23, 0x00, 0x03, 0x00, 0x04, CMD_R_PKT, 0x40, 0x00, 0x01};
...
TRY:	
    ret = ts->ops->send(hid_cmd, sizeof(hid_cmd));
    if(ret != sizeof(hid_cmd)) {
        dev_err(&client->dev,"[elan] %s send check gesture mode failed %d\n",__func__, ret);
        if(retry--)
            goto TRY;
    } else {
        ret = ts->ops->poll();
        if(ret) {
            dev_err(&client->dev, "[elan] polling int low failed\n", __func__);
            goto EXIT;
        }

        ret = ts->ops->recv(rbuf, sizeof(rbuf));
        if(ret != sizeof(rbuf)) {
            dev_err(&client->dev, "[elan] %s recv check gesture mode data failed %d\n", __func__, ret);
            goto EXIT;
        }else {
            dev_info(&client->dev, "[elan] %s %x:%x:%x:%x\n", __func__,rbuf[4],rbuf[5],rbuf[6],rbuf[7]);
            dev_info(&client->dev, "[elan] %s enter %d(rbuf[6]=1--gesture, rbuf[6]=0--normal) mode\n", __func__, rbuf[6]);
            ret = rbuf[6];
        }
    }
...
static int elan_gesture_enter_check(struct i2c_client *client, const int state)
{
    uint8_t hid_cmd[HID_CMD_LEN] = {0x04, 0x00, 0x23, 0x00, 0x03, 0x00, 0x04, CMD_W_PKT, 0x40, 0x01, 0x00};
    struct elan_ts_data *ts = i2c_get_clientdata(client);
    int ret = 0;
    int retry = 3;
    ...
    ret = elan_check_gesture(client);
    if(state  == ret) {
        dev_info(&client->dev, "[elan] %s current mode is %s\n", __func__, ret ==0x01? "gesture":"normal");
        elan_switch_irq(ts, 1);
        return 0;
    } else {
TRY:		
        ret = ts->ops->send(hid_cmd, sizeof(hid_cmd));
        if(ret != sizeof(hid_cmd)) {
            dev_err(&client->dev,"[elan] %s enter  gesture mode failed %d\n",__func__, ret);
            if(retry--)
                goto TRY;
        } else {
            ret = elan_check_gesture(client);
            if(state  == ret) {
                dev_info(&client->dev, "[elan] %s enter %d(ret=1--gesture, ret=0--normal) mode\n", __func__, ret);
                return 0;
            }
        }
    }
...
static int sprd_elan_ts_suspend(void)
{

    struct elan_ts_data *ts = elan_ts;
    int err = 0;
    int retry = RETRY_TIMES;

    //if do fw upgrade, don't sleep
    if (ts->power_lock == 0) {
        if(calibration_mode_flag == 1)
        {	
            elan_switch_irq(ts, 0);
            for(retry = 0 ; retry < RETRY_TIMES; retry++)
            {
                ...
                if (err != 0) {
                    dev_err(&ts->client->dev, "[elan] set power stats failed err = %d\n", err);
                } else {
					 dev_err(&ts->client->dev, "[elan] set power stats success err = %d, LINE = %d\n", err, __LINE__);
					 break;
				}
            }

        } else {
            dev_err(&ts->client->dev, "[elan] %s suspend flow \n", __func__);
            if(gesture_mode == 1){
                irq_set_irq_type(ts->hw_info.irq_num,IRQF_TRIGGER_LOW | IRQF_ONESHOT);
                printk("%s,%d\n",__func__,__LINE__); 
                if(enable_irq_wake(ts->hw_info.irq_num)){
                    dev_err(&ts->client->dev,"%s enable_irq_wake(irq:%d) fail",__func__, ts->hw_info.irq_num);
                }

#ifdef GESTURE_MODE
                err = elan_gesture_enter_check(ts->client, 0x01);
#endif
                if(err == 0){
                    dev_err(&ts->client->dev,"%s gesture mode enter OK\n", __func__);
                }
            }else{
                printk("%s,%d\n",__func__,__LINE__); 
                elan_switch_irq(ts, 0);
        ...
        dev_err(&ts->client->dev, "[elsn] %s Nothing Done!!\n",__func__);
    }
...
static int sprd_elan_ts_resume(void)
{
    struct elan_ts_data *ts = elan_ts;
    int err = 0;
    int retry = RETRY_TIMES;
     ...
     **/
    if (ts->power_lock == 0) {
        dev_err(&ts->client->dev, "[elan] reset gpio to resum tp\n");

        if(gesture_mode == 1)
        {
                irq_set_irq_type(ts->hw_info.irq_num,IRQF_TRIGGER_FALLING | IRQF_ONESHOT);
            if (disable_irq_wake(ts->hw_info.irq_num)) {
                dev_err(&ts->client->dev,"%s disable_irq_wake(irq:%d) fail",__func__, ts->hw_info.irq_num);
            }
...
reset_power_state:
            ...
            if (err) {
                dev_err(&ts->client->dev, "[elan]%s set power stata failed!!\n",__func__);
                if ((retry--) > 0)
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    return sprintf(buf, "TP FW version= 0x%04x\n",\
            ts->fw_info.fw_ver);
}
static ssize_t chip_id_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "chip id= EKTH7513\n");
}

static ssize_t ts_gesture_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    unsigned int flag;

    if (kstrtouint(buf, 10, &flag))
        return -EINVAL;
    if (flag == 1)
        gesture_mode = 1;
    else 
...
static ssize_t ts_gesture_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "gesture mode = 0x%04x\n",\
            gesture_mode);

}

static DEVICE_ATTR(ts_suspend, 0664, NULL, ts_suspend_store);
static DEVICE_ATTR(firmware_version, 0664, tp_version_show, NULL);
static DEVICE_ATTR(chip_id, 0664, chip_id_show, NULL);
static DEVICE_ATTR(gesture, 0664, ts_gesture_show, ts_gesture_store);
...
static void elan_gesture_report(struct elan_ts_data *ts, uint8_t *buf)
{
    ...
    switch(buf[3]) {
        case DOUBLE_CLICK:
            input_report_key(ts->finger_idev, KEY_POWER, 1);
            input_sync(ts->finger_idev);
            input_report_key(ts->finger_idev, KEY_POWER, 0);
            input_sync(ts->finger_idev);
            break;
        default:
            dev_err(&ts->client->dev,"[elan] %s undefine gesture packet %02x\n",__func__, buf[3]);
            break;
    }
...
static int elan_ts_recv_data(struct elan_ts_data *ts, uint8_t *buf)
{
    ...
    int i = 0;
    static int recv_size = sizeof(rbuf);


	
    rc = ts->ops->recv(rbuf, recv_size);
    if ( rc < 0) {
        dev_err(&ts->client->dev, "[elan] recv report data error [%d] !!\n",rc);
        return -1;
    } /*else {
        for(i = 0; i < HID_RECV_LEN/8 + 1; i++) {
        print_log(ts->level,"first paceket %02x %02x %02x %02x %02x %02x %02x %02x\n",\
        rbuf[i*8+0],rbuf[i*8+1],rbuf[i*8+2],rbuf[i*8+3],\
        rbuf[i*8+4],rbuf[i*8+5],rbuf[i*8+6],rbuf[i*8+7]);
        }
        }*/

    if( GESTURE_HEAD == rbuf[2]) {
        elan_gesture_report(ts, rbuf);
        return rc;
    }
    ...
    if ( report->tool_type == ELAN_PEN) {
		...
		recv_size = 0x13;
		print_log(ts->level, "[elan] ELAN_PEN recv size %d!!\n",recv_size);
    } else {
		...
		recv_size = sizeof(rbuf);
		print_log(ts->level, "[elan] HID_FID recv size %d!!\n",recv_size);
    }
...
static void  elants_a_report(struct elan_ts_data *ts, uint8_t *buf)
{
    ...
    if (report->tool_type == ELAN_FINGER) {
        for (reportid = 0; reportid < finger.fvalid_num; reportid++ ) {
            if (finger.fid ==  HID_FID) {  /*hid over i2c protocol*/
                ...
                if (fbits) {
					...
					print_log(ts->level, "[elan] finger  X:Y:INDEX down ====%d:%d:%d\n",x,y,finger.fid);
        ...
        if (stylus.key > 0 || !pkey) {
            switch(stylus.key) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    if(pkey != 0 && pkey != BTN_TOOL_RUBBER)
                    {
                        input_report_key(ts->pen_idev, pkey , 0);
                        input_sync(ts->pen_idev);
        ...
        if (stylus.inrange_status) {
            ...
            input_report_key(ts->pen_idev, BTN_TOUCH, stylus.inrange_status);
            input_report_key(ts->pen_idev, BTN_TOOL_PEN, stylus.inrange_status);
            input_sync(ts->pen_idev);
        } 
        else {
            input_report_key(ts->pen_idev, BTN_TOUCH, stylus.inrange_status);
...
static void elants_slot_report(struct elan_ts_data *ts, uint8_t *buf)
{
    ...
    if (report->tool_type == ELAN_FINGER) {
        ...
        if (finger.fid ==  HID_FID) {  /*hid over i2c protocol*/
            ...
            //if(num == 0)
               // input_report_key(ts->finger_idev, BTN_TOUCH, 0);
		    input_mt_sync_frame(ts->finger_idev);
            input_sync(ts->finger_idev);
		...
		input_mt_sync_frame(ts->pen_idev);
        input_sync(ts->pen_idev);
    }
...
static int touch_event_handler(void *unused)
{
	...
	//	dev_dbg(&ts->client->dev, "[elan] %s work!\n", __func__);
		...
		wait_event_interruptible(waiter, ts->tpd_flag != 0);
		ts->tpd_flag = 0;
		set_current_state(TASK_RUNNING);
		
		if(gpio_get_value(ts->hw_info.intr_gpio)) {
		    dev_dbg(&ts->client->dev,"[elan]interrupt jitter\n.");
			continue;
		}

		rc = elan_ts_recv_data(ts,buf);
		if (rc < 0) {
			dev_dbg(&ts->client->dev,"[elan]recv data error\n");
			continue;
		}

		if(ts->report.finger.recv_2nd_packet) {
			print_log(ts->level, "[elan]recv 2nd packet\n");
			continue;
		}
...
#ifdef GESTURE_MODE
    __set_bit(KEY_POWER,ts->finger_idev->keybit);
#endif
...
static int elan_ts_gpio_initial(struct ts_chip_hw_info *hw_info)
{
    ...
    gpio_direction_output(hw_info->rst_gpio, 0);
    ...
    //  ret = gpio_get_value(hw_info->rst_gpio);

    //   printk("elan  reset gpio value = %d\n", ret);
    //   
    //  ret = gpio_get_value(hw_info->intr_gpio);

    //   printk("elan  int gpio value = %d\n", ret);
        ...
        struct ts_chip_hw_info *chip_hw_info)
{
    ...
    chip_hw_info->intr_gpio = of_get_named_gpio(np,
            "elan,irq-gpio", 0);
    ...
    chip_hw_info->rst_gpio = of_get_named_gpio(np,
            "elan,rst-gpio", 0);
...
#endif
    elan_ts = ts;
    tp_sysfs_init(ts->client);
    get_cali_mode(calibra_mode);
    printk("%s,%d,calibra_mode = %s\n",__func__,__LINE__,calibra_mode);
    if(!strcmp(calibra_mode, calibration_mode)) 
    {
...
void elan_release_point(void)
{
    ...
    if (private_ts->finger_idev && private_ts->pen_idev) {
        ...
        if (private_ts->report_type == PROTOCOL_TYPE_B) {
            ...
            input_report_key(fidev, BTN_TOOL_FINGER, false);
...
int elan_ts_set_power_state(struct i2c_client *client, int state)
{
    int err = 0;
    struct elan_ts_data *ts = i2c_get_clientdata(client);
    ...
    if (ts->chip_type == HID_TYPE_PROTOCOL) {
        ...
        if (err != sizeof(hid_cmd)) {
            dev_err(&client->dev, "[elan] set power stats return %d\n", err);
            err = -EINVAL;
    ...
    dev_err(&client->dev,"[elan] set power stats success\n");
    return 0;
...
#ifdef GESTURE_MODE
        if(enable_irq_wake(ts->hw_info.irq_num)){
            dev_err(&client->dev,"%s enable_irq_wake(irq:%d) fail",__func__, ts->hw_info.irq_num);
        }

        err = elan_gesture_enter_check(client, 0x01);
        if(err == 0){
            dev_err(&client->dev,"%s gesture mode enter OK\n", __func__);
        }
#else 		
        elan_switch_irq(ts, 0);
...
#ifdef GESTURE_MODE
        if (disable_irq_wake(ts->hw_info.irq_num)) {
            dev_err(&client->dev,"%s disable_irq_wake(irq:%d) fail",__func__, ts->hw_info.irq_num);
        }
        elan_ts_hw_reset(&ts->hw_info);
#else
        /*device power on*/		
