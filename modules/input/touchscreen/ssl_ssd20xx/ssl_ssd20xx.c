...
static ssize_t solomon_firmwareid_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct solomon_device *ftdev = dev_get_drvdata(dev);
...
static DEVICE_ATTR(firmware_version, 0664, solomon_firmwareid_show, NULL);

static ssize_t solomon_input_name_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n",SOLOMON_NAME);
}

static DEVICE_ATTR(input_name, 0664, solomon_input_name_show, NULL);
//#####################################################################
