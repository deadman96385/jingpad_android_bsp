...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "FLASH_AW3642: %d %d %s : " fmt, current->pid, __LINE__, __func__

#define FLASH_IC_DRIVER_NAME       "sprd_aw3642"
...
static const char *const flash_ic_gpio_names[FLASH_IC_GPIO_MAX] = {
	...
	"flash-torch-en-gpios", /*gpio-157*/
	"flash-en-gpios", /*gpio-142 connect to strobe*/
...
static int sprd_flash_ic_init(void *drvd)
{
	...
	if (gpio_is_valid(gpio_id)) {
		...
		if (ret)
			pr_info("flash ic init gpio id %d err\n", gpio_id);
	}
	msleep(2);
	if (drv_data->i2c_info) {
		    ret = flash_ic_driver_reg_write(drv_data->i2c_info, 0x01, 0x00),/* bit[7]IVFM, [6]TXI, [5]Flash_pin, [4]Torch_pin, 
                              [1:0]  00:Shutdown  01: Indicator 10:Torch mode 11: Flash mode*/
		    ret = flash_ic_driver_reg_write(drv_data->i2c_info, 0x08, 0x1f),/* Flash Ramp time=1ms, Flash timeout=600ms*/
...
static int sprd_flash_ic_open_torch(void *drvd, uint8_t idx)
{
	...
	if (drv_data->i2c_info) {
		u8 data = 0;
		flash_ic_driver_reg_read(drv_data->i2c_info, 0x05, &data);
		pr_info("sprd_flash_ic_open_torch  Read value:%d",data);
		if (data != 0x96){
			flash_ic_driver_reg_write(drv_data->i2c_info,
						0x05, 0x80 | 0x16);
			flash_ic_driver_reg_read(drv_data->i2c_info, 0x05, &data);
			pr_info("sprd_flash_ic_open_torch  Write value:%d",data);
		}
		flash_ic_driver_reg_write(drv_data->i2c_info, 0x01, 0x0b);
	}
...
static int sprd_flash_ic_open_preflash(void *drvd, uint8_t idx)
{
	...
	if (drv_data->i2c_info) {
		u8 data = 0;
		flash_ic_driver_reg_read(drv_data->i2c_info, 0x05, &data);
		pr_info("sprd_flash_ic_open_preflash  Read value:%d",data);
		flash_ic_driver_reg_write(drv_data->i2c_info, 0x01, 0x0b);
		flash_ic_driver_reg_read(drv_data->i2c_info, 0x0a, &data);
		pr_info("sprd_flash_ic_open_preflash  Read 0x0a value:%d",data);
		flash_ic_driver_reg_read(drv_data->i2c_info, 0x0b, &data);
		pr_info("sprd_flash_ic_open_preflash  Read 0x0b value:%d",data);
	}
...
static int sprd_flash_ic_open_highlight(void *drvd, uint8_t idx)
{
	...
	if (drv_data->i2c_info) {
		...
		flash_ic_driver_reg_write(drv_data->i2c_info, 0x01, 0x0f);
		pr_info("sprd_flash_ic_open_highlight, read value:%d", data);
		flash_ic_driver_reg_read(drv_data->i2c_info, 0x0a, &data);
		pr_info("sprd_flash_ic_open_preflash  Read 0x0a value:%d",data);
		flash_ic_driver_reg_read(drv_data->i2c_info, 0x0b, &data);
		pr_info("sprd_flash_ic_open_preflash  Read 0x0b value:%d",data);
					  ...
					  struct sprd_flash_element *element)
{
	...
	if (drv_data->i2c_info) {
			...
			flash_ic_driver_reg_write(drv_data->i2c_info,
					0x05, 0x80 | 0x16);
			pr_info("write value:%d, read value:%d",(0x80 | 0x16), data);
		}
					  ...
					  struct sprd_flash_element *element)
{
	...
	if (drv_data->i2c_info) {
		...
		flash_ic_driver_reg_read(drv_data->i2c_info, 0x05, &data);
		flash_ic_driver_reg_write(drv_data->i2c_info,
				0x05, 0x80 | ( ((element->index * 3) < 0x2A)? (element->index * 3):0x2A));
	}
					   ...
					   struct sprd_flash_element *element)
{
	...
	if (drv_data->i2c_info) {
		...
		flash_ic_driver_reg_read(drv_data->i2c_info, 0x03, &data);
		flash_ic_driver_reg_write(drv_data->i2c_info,
				0x03, 0x80 | ( ((element->index * 3) < 0x2A)?(element->index * 3):0x2A));
	}
...
MODULE_DESCRIPTION("Sprd aw3642 Flash Driver");
MODULE_LICENSE("GPL");
