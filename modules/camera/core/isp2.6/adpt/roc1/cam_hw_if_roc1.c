...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "CAM_HW_IF_LROC1: %d %d %s : "\
...
static unsigned long fbc_store_base[FBC_PATH_NUM] = {
	ISP_FBC_STORE1_BASE,
	ISP_FBC_STORE2_BASE,
};
...
static const struct bypass_tag roc1_dcam_bypass_tab[] = {
...
static const struct bypass_tag roc1_isp_bypass_tab[] = {
...
static uint32_t roc1_cam_bypass_count_get(enum cam_bypass_type type)
{
	uint32_t cnt = 0;

	switch (type) {
	case DCAM_BYPASS_TYPE:
		cnt = sizeof(roc1_dcam_bypass_tab) /
			sizeof(roc1_dcam_bypass_tab[0]);
		break;
	case ISP_BYPASS_TYPE:
		cnt = sizeof(roc1_isp_bypass_tab) /
			sizeof(roc1_isp_bypass_tab[0]);
		break;
	default:
...
static struct bypass_tag *roc1_cam_bypass_data_get(uint32_t i,
	enum cam_bypass_type type)
{
	...
	switch (type) {
	case DCAM_BYPASS_TYPE:
		bypass = (struct bypass_tag *)&roc1_dcam_bypass_tab[i];
		break;
	case ISP_BYPASS_TYPE:
		bypass = (struct bypass_tag *)&roc1_isp_bypass_tab[i];
		break;
	default:
		pr_err("fail to support bypass type %d\n", type);
		break;
...
static uint32_t roc1_cam_reg_trace_tab[] = {
		DCAM_CFG,
		DCAM_APB_SRAM_CTRL,
		DCAM_IMAGE_CONTROL,
		DCAM_PDAF_CONTROL,
...
static void roc1_cam_reg_trace(uint32_t idx,
...
normal_reg_trace:
	...
	cnt = sizeof(roc1_cam_reg_trace_tab) /
		sizeof(roc1_cam_reg_trace_tab[0]);
	...
	for (i = 0; i < cnt; i += 8) {
		...
		n = ((cnt - i) < 8) ? (cnt - i) : 8;
		for (j = 0; j < n; j++) {
			addr = roc1_cam_reg_trace_tab[i + j];
...
static int roc1_dcam_clk_eb(struct cam_hw_soc_info *hw)
{
	int ret = 0;
...
static int roc1_dcam_clk_dis(struct cam_hw_soc_info *hw)
{
	int ret = 0;
...
static int roc1_dcam_clk_update(struct cam_hw_soc_info *hw, void *arg)
{
	int ret = 0;
...
static void roc1_dcam_axi_init(void *arg)
{
	uint32_t time_out = 0;
...
static void roc1_dcam_qos_set(struct cam_hw_soc_info *hw)
{
	uint32_t reg_val = 0;
...
static int roc1_dcam_start(void *arg)
{
	int ret = 0;
	struct dcam_pipe_dev *dev = NULL;
	uint32_t idx = 0;
...
static int roc1_dcam_stop(void *arg)
{
	int ret = 0;
...
static int roc1_dcam_cap_disable(void *arg)
{
	int ret = 0;
	struct dcam_pipe_dev *dev = NULL;
	uint32_t idx = 0;
...
static void roc1_dcam_auto_copy(uint32_t id, void *arg)
{
	struct dcam_pipe_dev *dev = NULL;
...
static void roc1_dcam_force_copy(uint32_t id, void *arg)
{
	struct dcam_pipe_dev *dev = NULL;
	const uint32_t bitmap[] = {
...
static int roc1_dcam_reset(struct cam_hw_info *hw, void *arg)
{
	int ret = 0;
	uint32_t i = 0;
	enum dcam_id idx = 0;
...
static int roc1_dcam_sram_reset(struct cam_hw_info *hw, void *arg)
{
	int ret = 0;
	enum dcam_id idx = 0;
	struct cam_hw_soc_info *soc = NULL;
	...
	pr_info("DCAM%d: sram reset end\n", idx);

	return ret;
}

static int roc1_dcam_path_pause(uint32_t idx, uint32_t path_id)
{
	switch (path_id) {
	case DCAM_PATH_FULL:
...
static int roc1_dcam_path_resume(uint32_t idx, uint32_t path_id)
{
	switch (path_id) {
	case DCAM_PATH_FULL:
...
static int roc1_dcam_fetch_set(void *arg)
{
	int ret = 0;
	...
	DCAM_REG_MWR(dev->idx, DCAM_INT_EN,
		DCAMINT_IRQ_LINE_MASK, DCAMINT_IRQ_LINE_MASK);
	/* Need to check need or not: this part roc1 like L3 or L5pro? */
	/*
...
static int roc1_dcam_mipi_cap_set(void *arg)
{
	int ret = 0;
	uint32_t idx = 0;
	uint32_t reg_val;
...
static int roc1_dcam_path_start(void *handle, uint32_t path_id)
{
	int ret = 0;
	uint32_t idx;
	struct dcam_pipe_dev *dev = NULL;
...
static int roc1_dcam_path_stop(void *handle, uint32_t path_id)
{
	int ret = 0;
	uint32_t idx;
	uint32_t reg_val;
...
static void roc1_dcam_fetch_start(struct cam_hw_info *hw)
{
	if (!hw) {
		pr_err("fail to get invalid hw\n");
		return;
...
static int roc1_dcam_path_size_update(void *handle, void *arg)
{
	int ret = 0;
	uint32_t idx;
	uint32_t path_id;
...
static int roc1_dcam_full_path_src_sel(void *handle,
...
static int roc1_dcam_lbuf_share_set(enum dcam_id idx, uint32_t width)
{
	int i = 0;
	int ret = 0;
	uint32_t tb_w[] = {
...
static int roc1_dcam_slice_fetch_set(void *arg)
{
	int ret = 0;
	uint32_t fetch_pitch;
	struct dcam_fetch_info *fetch = NULL;
...
static int roc1_dcam_ebd_set(uint32_t idx, void *arg)
{
...
static void roc1_dcam_fbc_ctrl(uint32_t idx, int fbc_mode)
{
	if (fbc_mode == DCAM_FBC_FULL_10_BIT)
		fbc_mode = 0x2;
	DCAM_REG_MWR(idx, DCAM_PATH_ENDIAN, 0x3, fbc_mode);
}

static int roc1_dcam_fbc_addr_set(uint32_t idx,
	unsigned long addr, void *arg)
{
	...
	DCAM_REG_WR(idx, addr, fbc_addr->addr2);

	return 0;
}

static int roc1_dcam_binning_4in1_set(void *arg, int binning_4in1_en)
{
	struct dcam_pipe_dev *dev = NULL;
...
static int roc1_dcam_sram_ctrl_set(void *arg, int sram_ctrl_en)
{
	struct dcam_pipe_dev *dev = NULL;
...
static int roc1_isp_clk_eb(struct cam_hw_soc_info *hw)
{
	int ret = 0;
...
static int roc1_isp_clk_dis(struct cam_hw_soc_info *hw)
{
	int ret = 0;
	pr_debug(",E\n");
	if (!hw) {
...
static int roc1_isp_clk_update(struct cam_hw_soc_info *hw, void *arg)
{
	int ret = 0;
...
int roc1_isp_reset(struct cam_hw_info *hw, void *arg)
{
	int rtn = 0;
	uint32_t cid;
	uint32_t time_out = 0;
...
int roc1_isp_irq_enable(struct cam_hw_ip_info *hw, void *arg)
{
	uint32_t ctx_id;
	uint32_t mask = ~0;
...
int roc1_isp_irq_disable(struct cam_hw_ip_info *hw, void *arg)
{
	uint32_t ctx_id;
...
int roc1_isp_irq_clear(struct cam_hw_ip_info *hw, void *arg)
{
	uint32_t ctx_id;
...
uint32_t *roc1_isp_cfg_map_info_get(void *arg)
{
	if (!arg) {
		pr_err("fail to get valid arg\n");
		return NULL;
...
void roc1_isp_default_param_set(struct cam_hw_info *hw,
...
static void *roc1_block_func_get(uint32_t index, enum cam_block_type type)
{
...
static uint32_t roc1_isp_hist_enable_get(int ctx_id)
{
	uint32_t bypass = 0;
...
static void roc1_isp_fetch_set(void *arg)
{
	uint32_t en_3dnr;
...
static int roc1_isp_fmcu_available(uint32_t fmcu_id)
{
	return (fmcu_id > 0) ? 0 : 1;
}

static void roc1_isp_ifbc_addr_set(uint32_t idx,
	uint32_t spath_id, unsigned long *out_addr)
{
	struct compressed_addr *addr;
	unsigned long ifbc_addr = fbc_store_base[spath_id];
	addr = (struct compressed_addr *)out_addr;

	ISP_REG_WR(idx, ifbc_addr +
		ISP_FBC_STORE_SLICE_YHEADER, addr->addr1);
	ISP_REG_WR(idx, ifbc_addr +
		ISP_FBC_STORE_SLICE_YADDR, addr->addr1);
	ISP_REG_WR(idx, ifbc_addr +
		ISP_FBC_STORE_SLICE_CHEADER, addr->addr2);
	ISP_REG_WR(idx, ifbc_addr +
		ISP_FBC_STORE_SLICE_CADDR, addr->addr2);
}

static int roc1_isp_ifbc_path_set(void *arg)
{
	int ret = 0;
	uint32_t val = 0;
	uint32_t idx = 0;
	struct isp_path_desc *path = NULL;
	struct isp_ifbc_store_info *ifbc_store_info = NULL;
	unsigned long addr = 0;
	...
	ifbc_store_info = &path->ifbc_store;
	addr = fbc_store_base[path->spath_id];

	pr_debug("isp set ifbc store in. bypass %d, path_id:%d, w:%d,h:%d\n",
			ifbc_store_info->bypass, path->spath_id,
			ifbc_store_info->size_in_hor, ifbc_store_info->size_in_ver);

	ISP_REG_MWR(idx, addr + ISP_FBC_STORE_PARAM,
		BIT_0, ifbc_store_info->bypass);
	if (ifbc_store_info->bypass)
		return 0;

	ISP_REG_MWR(idx, addr + ISP_FBC_STORE_PARAM,
		BIT_1, (ifbc_store_info->slice_mode_en << 1));
	ISP_REG_MWR(idx, addr+ISP_FBC_STORE_PARAM,
		BIT_3, (ifbc_store_info->mirror_en << 3));
	ISP_REG_MWR(idx, addr+ISP_FBC_STORE_PARAM,
		0xF0, (ifbc_store_info->color_format << 4));
	ISP_REG_MWR(idx, addr+ISP_FBC_STORE_PARAM,
		0x300, (ifbc_store_info->endian << 8));

	val = (ifbc_store_info->size_in_hor & 0x1FFF) |
		((ifbc_store_info->size_in_ver & 0x1FFF) << 16);
	ISP_REG_WR(idx, addr + ISP_FBC_STORE_SLICE_SIZE, val);

	val = (ifbc_store_info->border.up_border & 0xFF) |
		((ifbc_store_info->border.down_border & 0xFF) << 8) |
		((ifbc_store_info->border.left_border & 0xFF) << 16) |
		((ifbc_store_info->border.right_border & 0xFF) << 24);
	ISP_REG_WR(idx, addr + ISP_FBC_STORE_BORDER, val);

	val = ifbc_store_info->func_bits & 0xFFFF;
	ISP_REG_WR(idx, addr + ISP_FBC_STORE_P0, val);

	val = ifbc_store_info->tile_number_pitch & 0xFFFF;
	ISP_REG_WR(idx, addr + ISP_FBC_STORE_TILE_PITCH, val);

	val = ifbc_store_info->fbc_constant_yuv;
	ISP_REG_WR(idx, addr + ISP_FBC_STORE_CONSTANT, val);

	val = ifbc_store_info->tile_number & 0xFFFFF;
	ISP_REG_WR(idx, addr + ISP_FBC_STORE_TILE_NUM, val);

	val = 0x20002 & 0xFF00FF;
	ISP_REG_MWR(idx, addr+ISP_FBC_STORE_NFULL_LEVEL, 0xFF00FF, val);

	return ret;
}

static int roc1_isp_fbd_slice_set(void *fmcu_handle, void *arg)
{
...
static void roc1_isp_fbd_addr_set(int idx, void *arg1, void *arg2)
{
	uint32_t addr = 0;
...
static void roc1_isp_ifbc_fmcu_addr_set(void *fmcu_handle,
	void *arg, int index)
{
	uint32_t addr = 0, cmd = 0;
	struct isp_fmcu_ctx_desc *fmcu = NULL;
	struct slice_ifbc_store_info *ifbc_store = NULL;
	unsigned long ifbc_base = fbc_store_base[index];
	...
	fmcu = (struct isp_fmcu_ctx_desc *)fmcu_handle;
	ifbc_store = (struct slice_ifbc_store_info *)arg;

	if (!ifbc_store->slice_mode_en)
		return;

	addr = ISP_GET_REG(ISP_FBC_STORE_SLICE_YADDR) + ifbc_base;
	cmd = ifbc_store->fbc_y_tile_addr_init_x256;
	FMCU_PUSH(fmcu, addr, cmd);

	addr = ISP_GET_REG(ISP_FBC_STORE_SLICE_CADDR) + ifbc_base;
	cmd = ifbc_store->fbc_c_tile_addr_init_x256;
	FMCU_PUSH(fmcu, addr, cmd);

	addr = ISP_GET_REG(ISP_FBC_STORE_SLICE_YHEADER) + ifbc_base;
	cmd = ifbc_store->fbc_y_header_addr_init;
	FMCU_PUSH(fmcu, addr, cmd);

	addr = ISP_GET_REG(ISP_FBC_STORE_SLICE_CHEADER) + ifbc_base;
	cmd = ifbc_store->fbc_c_header_addr_init;
	FMCU_PUSH(fmcu, addr, cmd);
}

static int roc1_isp_ifbc_path_slice_set(void *fmcu_handle,
	uint32_t path_en, uint32_t ctx_idx, uint32_t spath_id, void *arg)
{
	uint32_t addr = 0, cmd = 0;
	struct isp_fmcu_ctx_desc *fmcu = NULL;
	struct slice_ifbc_store_info *slc_ifbc_store = NULL;
	unsigned long ifbc_base = fbc_store_base[spath_id];
	...
	fmcu = (struct isp_fmcu_ctx_desc *)fmcu_handle;
	slc_ifbc_store = (struct slice_ifbc_store_info *)arg;

	if (!path_en) {
		/* bit0 bypass store */
		addr = ap_fmcu_reg_get(fmcu, ISP_FBC_STORE_PARAM) + ifbc_base;
		cmd = 1;
	...
	addr = ap_fmcu_reg_get(fmcu, ISP_FBC_STORE_PARAM) + ifbc_base;
	cmd = ISP_REG_RD(ctx_idx, ifbc_base + ISP_FBC_STORE_PARAM);
	cmd |= (slc_ifbc_store->slice_mode_en & 0x1) << 1;
	ap_fmcu_reg_write(fmcu, ctx_idx, addr, cmd);

	addr = ap_fmcu_reg_get(fmcu, ISP_FBC_STORE_SLICE_SIZE) + ifbc_base;
	cmd = ((slc_ifbc_store->fbc_size_in_ver & 0xFFFF) << 16) |
			(slc_ifbc_store->fbc_size_in_hor & 0xFFFF);
	ap_fmcu_reg_write(fmcu, ctx_idx, addr, cmd);

	addr = ap_fmcu_reg_get(fmcu, ISP_FBC_STORE_BORDER) + ifbc_base;
	cmd = (slc_ifbc_store->border.up_border& 0xFF) |
			((slc_ifbc_store->border.down_border& 0xFF) << 8) |
			((slc_ifbc_store->border.left_border& 0xFF) << 16) |
			((slc_ifbc_store->border.right_border & 0xFF) << 24);
	ap_fmcu_reg_write(fmcu, ctx_idx, addr, cmd);

	addr = ap_fmcu_reg_get(fmcu,
		ISP_FBC_STORE_SLICE_YHEADER) + ifbc_base;
	cmd = slc_ifbc_store->fbc_y_tile_addr_init_x256;
	ap_fmcu_reg_write(fmcu, ctx_idx, addr, cmd);

	addr = ap_fmcu_reg_get(fmcu,
		ISP_FBC_STORE_SLICE_CHEADER) + ifbc_base;
	cmd = slc_ifbc_store->fbc_c_tile_addr_init_x256;
	ap_fmcu_reg_write(fmcu, ctx_idx, addr, cmd);

	addr = ap_fmcu_reg_get(fmcu,
		ISP_FBC_STORE_SLICE_YADDR) + ifbc_base;
	cmd = slc_ifbc_store->fbc_y_header_addr_init;
	ap_fmcu_reg_write(fmcu, ctx_idx, addr, cmd);

	addr = ap_fmcu_reg_get(fmcu,
		ISP_FBC_STORE_SLICE_CADDR) + ifbc_base;
	cmd = slc_ifbc_store->fbc_c_header_addr_init;
	ap_fmcu_reg_write(fmcu, ctx_idx, addr, cmd);

	addr = ap_fmcu_reg_get(fmcu,
		ISP_FBC_STORE_TILE_NUM) + ifbc_base;
	cmd = slc_ifbc_store->fbc_tile_number;
	ap_fmcu_reg_write(fmcu, ctx_idx, addr, cmd);
...
static int roc1_isp_ltm_slice_set(
		void *fmcu_handle,
...
void roc1_isp_cfg_subblock(void *ctx)
{
	uint32_t idx = 0;
	struct isp_pipe_context *pctx = NULL;
	struct isp_fetch_info *fetch = NULL;
...
void roc1_isp_yuv_block_ctrl(uint32_t idx, void *arg,
	enum isp_yuv_block_ctrl_type type)
{
	struct isp_k_block *param = NULL;
...
BLOCK_CFG:
	ISP_REG_MWR(idx, ISP_CCE_PARAM, BIT_0, param->cce_bypass);
	ISP_REG_MWR(idx, ISP_BCHS_PARAM, BIT_0, param->bchs_bypass);
	ISP_REG_MWR(idx, ISP_CDN_PARAM, BIT_0, param->cdn_bypass);
	ISP_REG_MWR(idx, ISP_EE_PARAM, BIT_0, param->edge_bypass);
	ISP_REG_MWR(idx, ISP_HIST2_PARAM, BIT_0, param->hist_bypass);
	ISP_REG_MWR(idx, ISP_IIRCNR_PARAM, BIT_0, param->iircnr_bypass);
	ISP_REG_MWR(idx, ISP_YUV_NF_CTRL, BIT_0, param->nf_bypass);
	ISP_REG_MWR(idx, ISP_POSTCDN_COMMON_CTRL,
		BIT_0, param->postcdn_bypass);
	ISP_REG_MWR(idx, ISP_PRECDN_PARAM, BIT_0, param->precdn_bypass);
	ISP_REG_MWR(idx, ISP_PSTRZ_PARAM, BIT_0, param->pstz_bypass);
	ISP_REG_MWR(idx, ISP_UVD_PARAM, BIT_0, param->uvd_bypass);
	ISP_REG_MWR(idx, ISP_YGAMMA_PARAM, BIT_0, param->ygamma_bypass);
	ISP_REG_MWR(idx, ISP_YNR_CONTRL0, BIT_0, param->ynr_bypass);
	ISP_REG_MWR(idx, ISP_YRANDOM_PARAM1, BIT_0, param->yrandom_bypass);
	return;
...
static uint32_t roc1_path_ctrl_id[DCAM_PATH_MAX] = {
...
static unsigned long roc1_dcam_store_addr[DCAM_PATH_MAX] = {
...
static unsigned long roc1_dcam2_store_addr[DCAM_PATH_MAX] = {
	[DCAM_PATH_FULL] = DCAM2_PATH0_BASE_WADDR,
	[DCAM_PATH_BIN] = DCAM2_PATH1_BASE_WADDR,
};

static uint32_t roc1_isp_ctx_fmcu_support[ISP_CONTEXT_HW_NUM] = {
	[ISP_CONTEXT_HW_P0] = 1,
	[ISP_CONTEXT_HW_C0] = 1,
	[ISP_CONTEXT_HW_P1] = 1,
	[ISP_CONTEXT_HW_C1] = 1,
...
static struct cam_hw_soc_info roc1_dcam_soc_info;
static struct cam_hw_soc_info roc1_isp_soc_info;
static struct cam_hw_ip_info roc1_dcam[DCAM_ID_MAX] = {
	[DCAM_ID_0] = {
		...
		.store_addr_tab = roc1_dcam_store_addr,
		.path_ctrl_id_tab = roc1_path_ctrl_id,
	...
	[DCAM_ID_2] = {
		...
		.store_addr_tab = roc1_dcam2_store_addr,
		.path_ctrl_id_tab = roc1_path_ctrl_id,
		.pdaf_type3_reg_addr = DCAM_PPE_RIGHT_WADDR,
		.rds_en = 0,
	},
...
static struct cam_hw_ip_info roc1_isp = {
	.slm_cfg_support = 1,
	.ctx_fmcu_support = roc1_isp_ctx_fmcu_support,
};

struct cam_hw_info roc1_hw_info = {
	.prj_id = ROC1,
	.pdev = NULL,
	.soc_dcam = &roc1_dcam_soc_info,
	.soc_isp = &roc1_isp_soc_info,
	.ip_dcam[DCAM_ID_0] = &roc1_dcam[DCAM_ID_0],
	.ip_dcam[DCAM_ID_1] = &roc1_dcam[DCAM_ID_1],
	.ip_dcam[DCAM_ID_2] = &roc1_dcam[DCAM_ID_2],
	.ip_isp = &roc1_isp,
	.hw_ops = {
		.dcam_soc_ops = {
			.clk_enable = roc1_dcam_clk_eb,
			.clk_disable = roc1_dcam_clk_dis,
			.clk_update = roc1_dcam_clk_update,
			.axi_init = roc1_dcam_axi_init,
			.qos_set = roc1_dcam_qos_set,
			.reset = roc1_dcam_reset,
			.sram_reset = roc1_dcam_sram_reset,
		},
		.isp_soc_ops = {
			.clk_enable = roc1_isp_clk_eb,
			.clk_disable = roc1_isp_clk_dis,
			.clk_update = roc1_isp_clk_update,
			.axi_init = NULL,
			.qos_set = NULL,
			.reset = roc1_isp_reset,
		},
		.dcam_irq_ops = {
			.irq_request = NULL,
			.irq_free = NULL,
		...
		.isp_irq_ops = {
			.irq_request = NULL,
			.irq_free = NULL,
			.irq_enable = roc1_isp_irq_enable,
			.irq_disable = roc1_isp_irq_disable,
			.irq_clear = roc1_isp_irq_clear,
			.irq_proc = NULL,
		},
		.core_ops = {
			.start = roc1_dcam_start,
			.stop = roc1_dcam_stop,
			.stop_cap_eb = roc1_dcam_cap_disable,
			.auto_copy = roc1_dcam_auto_copy,
			.force_copy = roc1_dcam_force_copy,
			.path_start = roc1_dcam_path_start,
			.path_stop = roc1_dcam_path_stop,
			.path_pause = roc1_dcam_path_pause,
			.path_resume = roc1_dcam_path_resume,
			.mipi_cap_set = roc1_dcam_mipi_cap_set,
			.fetch_start = roc1_dcam_fetch_start,
			.path_size_update = roc1_dcam_path_size_update,
			.dcam_calc_rds_phase_info = NULL,
			.path_src_sel = roc1_dcam_full_path_src_sel,
			.default_para_set = roc1_isp_default_param_set,
			.lbuf_share_set = roc1_dcam_lbuf_share_set,
			.dcam_slice_fetch_set = roc1_dcam_slice_fetch_set,
			.ebd_set = roc1_dcam_ebd_set,
			.binning_4in1_set = roc1_dcam_binning_4in1_set,
			.sram_ctrl_set = roc1_dcam_sram_ctrl_set,
			.dcam_fetch_set = roc1_dcam_fetch_set,
			.dcam_fbc_ctrl = roc1_dcam_fbc_ctrl,
			.dcam_fbc_addr_set = roc1_dcam_fbc_addr_set,
			.dcam_gtm_status_get = NULL,
			.cam_gtm_ltm_eb = NULL,
			.cam_gtm_ltm_dis = NULL,
			.cam_gtm_update = NULL,
			.isp_fetch_set = roc1_isp_fetch_set,
			.isp_fbd_slice_set = roc1_isp_fbd_slice_set,
			.isp_fbd_addr_set = roc1_isp_fbd_addr_set,
			.isp_afbc_addr_set = NULL,
			.isp_afbc_path_set = NULL,
			.isp_afbc_fmcu_addr_set = NULL,
			.isp_afbc_path_slice_set = NULL,
			.isp_ifbc_addr_set = roc1_isp_ifbc_addr_set,
			.isp_ifbc_path_set = roc1_isp_ifbc_path_set,
			.isp_ifbc_fmcu_addr_set = roc1_isp_ifbc_fmcu_addr_set,
			.isp_ifbc_path_slice_set = roc1_isp_ifbc_path_slice_set,
			.isp_ltm_slice_set = roc1_isp_ltm_slice_set,
			.isp_nr3_fbc_slice_set = set_slice_3dnr_fbc_store,
			.isp_nr3_fbd_slice_set = set_slice_3dnr_fbd_fetch,
			.hist_enable_get = roc1_isp_hist_enable_get,
			.block_func_get = roc1_block_func_get,
			.cfg_map_info_get = roc1_isp_cfg_map_info_get,
			.fmcu_valid_get = roc1_isp_fmcu_available,
			.bypass_data_get = roc1_cam_bypass_data_get,
			.bypass_count_get = roc1_cam_bypass_count_get,
			.reg_trace = roc1_cam_reg_trace,
			.isp_cfg_subblock = roc1_isp_cfg_subblock,
			.isp_yuv_block_ctrl = roc1_isp_yuv_block_ctrl,
		},
	},
};
