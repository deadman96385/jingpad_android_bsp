	...
	struct isp_k_block *isp_k_param, uint32_t idx)
{
	...
	isp_k_param->nf_bypass = nf_info.yrandom_bypass;
	...
	if (nf_info.shape_mode != 0) {
		...
		pr_debug("seed0_for_mode1=%d weight %d\n", isp_k_param->seed0_for_mode1,
			isp_k_param->src_w);
		sprd_noisefilter_seeds(isp_k_param->src_w, nf_info.yrandom_seed[0],
			&nf_info.yrandom_seed[1], &nf_info.yrandom_seed[2],
			&nf_info.yrandom_seed[3]);
	}
