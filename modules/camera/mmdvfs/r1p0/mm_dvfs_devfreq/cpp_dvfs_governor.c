...
MODULE_DEVICE_TABLE(of, cpp_dvfs_of_match);
...
static struct platform_driver cpp_dvfs_driver = {
    ...
    .driver =
        {
            .name = "cpp-dvfs", .of_match_table = cpp_dvfs_of_match,
        },
};
...
static int dvfs_probe(struct platform_device *pdev) {
    ...
    module->devfreq =
        devm_devfreq_add_device(dev, &dvfs_profile, "cpp_dvfs", NULL);
...
static struct attribute_group dev_attr_group = {
    .name = "cpp_governor", .attrs = dev_entries,
};

static struct attribute_group coeff_attr_group = {
    .name = "cpp_coeff", .attrs = coeff_entries,
};
...
static int userspace_init(struct devfreq *devfreq) {
    ...
    module->dvfs_ops = get_ip_dvfs_ops("CPP_DVFS_OPS");
