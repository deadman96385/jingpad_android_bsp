...
//static int syna_tcm_driver_removing = 0;
...
SHOW_STORE_PROTOTYPE(syna_tcm, ts_suspend)
SHOW_PROTOTYPE(syna_tcm, input_name)
...
static ssize_t syna_tcm_sysfs_ts_suspend_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
	struct device *p_dev;
...
static ssize_t syna_tcm_sysfs_ts_suspend_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	struct device *p_dev;
...
static ssize_t syna_tcm_sysfs_input_name_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", TOUCH_INPUT_NAME);
}
...
exit:
	...
	if (mod_pool.queue_work)
		schedule_delayed_work(&mod_pool.work, 0);
...
static irqreturn_t syna_tcm_isr(int irq, void *data)
{
	...
	retval = tcm_hcd->read_message(tcm_hcd,
			...
			0);
	if (retval < 0) {
        if (tcm_hcd->syna_tcm_driver_removing) goto exit;
...
static void syna_tcm_helper_work(struct work_struct *work)
{
    ...
    if (tcm_hcd->syna_tcm_driver_removing) return;
...
static int syna_tcm_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ERR ">>>>>>>>>>>%s start!<<<<<<<<<<<", __func__);
	...
	retval = sysfs_create_link(NULL, sysfs_dir, "touchscreen");
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "syna_create_sysfs failed!");
		retval = -EINVAL;
		goto err_sysfs_create_dir;
	}
...
prepare_modules:
	...
	INIT_DELAYED_WORK(&mod_pool.work, syna_tcm_module_work);
	mod_pool.tcm_hcd = tcm_hcd;
	mod_pool.queue_work = true;
	schedule_delayed_work(&mod_pool.work, 30 * HZ);

	printk(KERN_ERR ">>>>>>>>>>>%s end!<<<<<<<<<<<", __func__);
...
static int syna_tcm_remove(struct platform_device *pdev)
{
    ...
    tcm_hcd->syna_tcm_driver_removing = 1;
	...
	cancel_delayed_work_sync(&mod_pool.work);
