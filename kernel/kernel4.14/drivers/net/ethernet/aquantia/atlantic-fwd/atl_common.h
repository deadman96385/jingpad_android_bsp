...
/* ATL_RING_SIZE was 4096 original, it demands to
 * pre-allocate 32Mb for each RX and TX ring.
 * It is too large for Orca CPE, since the left memory on Orca CPE
 * is about 30Mb+, which will cause OOM on Orca CPE.
 * Thus, we modify it to 1024, and it only occupies 8Mb memory in total.
 * It can also be modified dynamically.
 * for example: ethtool -G eth0 rx 1024 tx 1024
 */
#define ATL_RING_SIZE 512
