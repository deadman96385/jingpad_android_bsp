...
static void sprd_mpm_print_awake(struct sprd_mpm_data *mpm)
{
	...
	if (len)
		...
		if (mpm->wakelock_cnt)
			pr_info("pcie res: wakelock_cnt err!");
	}
...
static void sprd_mpm_print_timer_fn(unsigned long data)
{
 ...
 */
static void sprd_mpm_deactive_timer_fn(unsigned long data)
{
	struct sprd_mpm_data *mpm = (struct sprd_mpm_data *)data;
 ...
 */
static void sprd_pms_relax_wakelock_timer(unsigned long data)
{
					    ...
					    unsigned int later)
{
	...
	pr_info("mpm: %s set later_idle = %d.\n", mpm->name, later);
