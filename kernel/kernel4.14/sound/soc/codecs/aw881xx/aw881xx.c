	...
	struct snd_soc_dai *dai)
{
	...
	aw881xx->startup_cnt++;
	pr_info("%s stream %d, startup_cnt %d\n", __func__, substream->stream, aw881xx->startup_cnt);
	...
	struct snd_soc_dai *dai)
{
	...
	/* get bit width */
	aw881xx->width = AW881XX_24BIT_WIDTH;
	pr_debug("%s: params_width %d, width = %d, rate %d\n", __func__,
		 params_width(params), aw881xx->width, aw881xx->rate);
				...
				struct device_node *np)
{
	...
	/* vbat monitor */
	ret = of_property_read_u32(np, "aw881xx,monitor-flag", &aw881xx->monitor_flag);
	if (ret) {
	...
	ret = of_property_read_u32(np, "aw881xx,monitor-timer-val",
...
int aw881xx_update_cfg_name(struct aw881xx *aw881xx)
{
	...
	pr_info("%s %d, buf %s, pid=%d\n", __func__, __LINE__, buf, aw881xx->pid);

	for (i = 0; i < sizeof(aw881xx_cfg_name)/AW881XX_CFG_NAME_MAX; i++)
		memcpy(aw881xx_cfg_name[i] + head_index, buf, strlen(buf));
...
static enum hrtimer_restart aw881xx_fw_timer_func(struct hrtimer *timer)
{
	struct aw881xx *aw881xx =
		container_of(timer, struct aw881xx, fw_timer);

	pr_debug("%s: enter\n", __func__);

	schedule_work(&aw881xx->fw_work);

	return HRTIMER_NORESTART;
}

static void aw881xx_fw_work_routine(struct work_struct *work)
{
	struct aw881xx *aw881xx =
		container_of(work, struct aw881xx, fw_work);
	struct device_ops *dev_ops = &(aw881xx->dev_ops);
...
static int aw881xx_fw_init(struct aw881xx *aw881xx)
{
	pr_info("%s: enter\n", __func__);

	hrtimer_init(&aw881xx->fw_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	aw881xx->fw_timer.function = aw881xx_fw_timer_func;
	INIT_WORK(&aw881xx->fw_work, aw881xx_fw_work_routine);
	...
	const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(aw881xx->irq_gpio) &&
		...
		ret = devm_request_threaded_irq(&i2c->dev,
			...
			NULL, aw881xx_irq, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
			"aw881xx", aw881xx);
		if (ret != 0) {
			dev_err(&i2c->dev,
...
static int aw881xx_i2c_remove(struct i2c_client *i2c)
{
	...
	if (hrtimer_active(&aw881xx->fw_timer)) {
		pr_info("%s: cancel fw timer\n", __func__);
		hrtimer_cancel(&aw881xx->fw_timer);
	}
