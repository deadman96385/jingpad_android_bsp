...
#define AW87XXX_DRIVER_VERSION	"v0.1.6"
static bool boot_calibration;
static bool boot_charger;
/*************************************************************************
 * aw87xxx variable
 ************************************************************************/
...
unsigned char aw87xxx_show_current_mode(int32_t channel)
{
	...
	list_for_each(pos, &g_aw87xxx_device_list) {
		...
		if (aw87xxx->driver_num == channel) {
			pr_info("%s enter, 2c%d@0x%02x = \n", __func__,
				aw87xxx->i2c_client->adapter->nr,
...
int aw87xxx_audio_scenne_load(uint8_t mode, int32_t channel)
{
	struct aw87xxx_scene_param *scene_param;
	struct list_head *pos;
	struct aw87xxx *aw87xxx;
				     ...
				     const struct firmware *cont)
{
	...
	aw87xxx_parse_chip_name(aw87xxx,
				(uint32_t *) current_scene->bin_file->
	...
	curr_scene_cnt->len =
	    current_scene->bin_file->header_info[0].bin_data_len;
	memcpy(curr_scene_cnt->data,
	       cont->data + current_scene->bin_file->header_info[0].header_len,
	       current_scene->bin_file->header_info[0].bin_data_len);
...
static void aw87xxx_old_parse_firmware(struct aw87xxx *aw87xxx,
...
static int aw87xxx_choice_device(struct aw87xxx *aw87xxx, uint8_t chip_type)
{
	...
	case AW87XXX_519:
		...
		break;
	case AW87XXX_549:
		aw87xxx->reg_param.reg_max = AW87549_REG_MAX;
		aw87xxx->reg_param.reg_access = AW87549_reg_access;
		aw87xxx->reg_param.reg_max = AW87559_REG_MAX;
		aw87xxx->reg_param.reg_access = AW87559_reg_access;
		aw87xxx->reg_param.reg_max = AW87569_REG_MAX;
		aw87xxx->reg_param.reg_access = AW87569_reg_access;
		aw87xxx->reg_param.reg_max = AW87579_REG_MAX;
		aw87xxx->reg_param.reg_access = AW87579_reg_access;
		break;
	default:
...
static void aw87xxx_parse_scene_mode_dt(struct aw87xxx *aw87xxx)
{
	...
	if (strstr(scene_mode_str, "music")) {
		aw_scene_ls->aw_music.cfg_update_flag = AW87XXX_CFG_WAIT;
	} else {
		aw_dev_info(aw87xxx->dev,
			    "%s:have not got secne-mode music\n", __func__);
	}
	if (strstr(scene_mode_str, "voice")) {
		aw_scene_ls->aw_voice.cfg_update_flag = AW87XXX_CFG_WAIT;
	} else {
		aw_dev_info(aw87xxx->dev,
			    "%s:have not got secne-mode voice\n", __func__);
	}
	if (strstr(scene_mode_str, "fm")) {
		aw_scene_ls->aw_fm.cfg_update_flag = AW87XXX_CFG_WAIT;
	} else {
		aw_dev_info(aw87xxx->dev,
			    "%s:have not get secne-mode fm\n", __func__);
	}
	if (strstr(scene_mode_str, "rcv")) {
		aw_scene_ls->aw_rcv.cfg_update_flag = AW87XXX_CFG_WAIT;
	} else {
		aw_dev_info(aw87xxx->dev,
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	pr_info("%s enter, i2c%d@0x%02x, channel = %d\n",
		__func__, aw87xxx->i2c_client->adapter->nr,
		aw87xxx->i2c_client->addr, aw87xxx->driver_num);
				...
				size_t len)
{
	...
	pr_info("%s enter, i2c%d@0x%02x\n", __func__,
		aw87xxx->i2c_client->adapter->nr, aw87xxx->i2c_client->addr);

	ret = kstrtouint(buf, 8, &mode);
	aw_dev_info(aw87xxx->dev, "%s enter, mode is %d\n", __func__, mode);
	if (ret) {
		aw_dev_err(aw87xxx->dev, "%s: fail to change str to int\n",
			   __func__);
		return ret;
	}
	ret = aw87xxx_audio_scenne_load((uint8_t) mode, aw87xxx->driver_num);
	if (ret) {
		aw_dev_err(aw87xxx->dev, "%s: load scene:%s failed",
			   __func__, buf);
	}
...
{
	int32_t temp_channel = aw87xxx->driver_num;
	...
	if (temp_channel >= 0) {
		snprintf(aw87xxx->channel, sizeof(aw87xxx->channel) + 1, "%d",
			 temp_channel);
		pr_info("%s: channel = %s", __func__, aw87xxx->channel);
	} else {
		aw87xxx->channel[0] = '-';
	}
}
				     ...
				     struct device_node *np)
{
	...
	if (aw87xxx->i2c_client->addr == 0x58) {
		aw87xxx->name_suffix = "58";
	} else if (aw87xxx->i2c_client->addr == 0x59) {
		aw87xxx->name_suffix = "59";
	} else if (aw87xxx->i2c_client->addr == 0x5a) {
		aw87xxx->name_suffix = "5a";
	} else if (aw87xxx->i2c_client->addr == 0x5b) {
		aw87xxx->name_suffix = "5b";
	} else {
		dev_info(aw87xxx->dev,
			 "%s:not stereo channel,use default single track\n",
			 __func__);
		goto chan_default;
	}

	ret = of_property_read_u32(np, "driver-num", &aw87xxx->driver_num);
	if (ret != 0) {
		pr_err("%s: driver_num ,user default value!, ret = %d\n",
		       __func__, ret);
		aw87xxx->driver_num = -1;
	} else {
		pr_info("%s: driver-num = %d\n", __func__, aw87xxx->driver_num);
	}
...
static int aw87xxx_update_cfg_name(struct aw87xxx *aw87xxx)
{
	...
	if (aw87xxx->channel[0] != '-') {
		...
		pr_info("%s:i2c%d@0x%02x -- vmax_name = %s\n",
			__func__, aw87xxx->i2c_client->adapter->nr,
			aw87xxx->i2c_client->addr, aw87xxx->vmax_cfg_name);
	} else {
		memcpy(aw87xxx->vmax_cfg_name, aw87xxx_vmax_cfg_name,
		       strlen(aw87xxx_vmax_cfg_name));
		memcpy(aw87xxx->vmax_cfg_name + strlen(aw87xxx->vmax_cfg_name) -
...
static int aw87xxx_read_chipid(struct aw87xxx *aw87xxx)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		...
		case AW87XXX_PID_5A:
			aw87xxx->chipid = reg_val;
			aw87xxx_choice_device(aw87xxx, AW87XXX_549);
			aw87xxx_update_cfg_name(aw87xxx);
			aw_dev_info(aw87xxx->dev,
			     ...
			     const struct i2c_device_id *id)
{
    ...
    /*when enter calibration mode, disable aw87xxx probe to fix device can't enter deepsleep on checkx test */
    if(boot_calibration || boot_charger)
        return 0;
	...
	aw_dev_info(&client->dev, "%s good nice!\n", __func__);
	...
	if (client->dev.of_node) {
		if (aw87xxx->name_suffix)
			dev_set_name(&client->dev, "%s_%s", "aw87xxx_pa",
				     aw87xxx->name_suffix);
		else
...
static int __init get_calibration_mode(char *str)
{
    if(!str)
        return 0;
...
__setup("androidboot.mode=", get_calibration_mode);
